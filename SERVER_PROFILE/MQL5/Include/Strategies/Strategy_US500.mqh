#ifndef STRATEGY_US500_INCLUDED
#define STRATEGY_US500_INCLUDED
...
int g_US500_ema_fast_handle = INVALID_HANDLE;
int g_US500_ema_slow_handle = INVALID_HANDLE;
int g_US500_atr_handle = INVALID_HANDLE;
int g_US500_rsi_handle = INVALID_HANDLE;
datetime g_US500_last_bar_time = 0;
double g_US500_last_atr_points = 0.0;
datetime g_US500_last_position_modify = 0;
MbTuningLocalPolicy g_US500_tuning_policy;

struct US500LocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildUS500Params()
  {
   MbFamilyStrategyParams out;
   out.family_kind = MB_FAMILY_KIND_INDEX_US;
   out.ema_fast_period = 11;
   out.ema_slow_period = 32;
   out.atr_period = 14;
   out.rsi_period = 14;
   ...
   out.max_risk_pct = 0.21;
   out.sl_atr_multiplier = 1.65;
   out.sl_min_points = 70.0;
   out.tp_atr_multiplier = 2.15;
   out.tp_min_points = 115.0;
   out.trail_atr_multiplier = 1.05;
   out.pressure_step_scale = 7.0;
   out.ready_trigger_abs = 0.84;
   out.caution_trigger_abs = 1.00;
   return out;
  }

bool StrategyUS500Init(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildUS500Params();
   bool ok = MbFamilyStrategyInit(profile,params,g_US500_ema_fast_handle,g_US500_ema_slow_handle,g_US500_atr_handle,g_US500_rsi_handle,g_US500_last_bar_time,g_US500_last_atr_points,g_US500_last_position_modify);
   MbTuningLocalPolicyReset(g_US500_tuning_policy);
   return ok;
  }

void StrategyUS500Deinit()
  {
   MbFamilyStrategyDeinit(g_US500_ema_fast_handle,g_US500_ema_slow_handle,g_US500_atr_handle,g_US500_rsi_handle,g_US500_last_bar_time,g_US500_last_atr_points,g_US500_last_position_modify);
  }

void StrategyUS500SetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_US500_tuning_policy = policy;
  }

void ApplyUS500TuningPenalty(
...
int ResolveUS500MinutesFromWindowStart(const MbSymbolProfile &profile)
  {
   MqlDateTime tm;
...
int ResolveUS500MinutesToWindowEnd(const MbSymbolProfile &profile)
  {
   MqlDateTime tm;
   TimeToStruct(TimeCurrent(),tm);
...
void BuildUS500RiskPlan(
   const MbRuntimeState &state,
   const MbMarketSnapshot &snapshot,
   US500LocalRiskPlan &out
)
  {
   MbFamilyStrategyParams params = BuildUS500Params();
   MbFamilyStrategyBuildRiskPlan(state,snapshot,g_US500_last_atr_points,params,out.allowed,out.lots,out.sl_points,out.tp_points,out.reason_code);
  }

void ManageUS500OpenPosition(
   CTrade &trade,
  ...
  {
   MbFamilyStrategyParams params = BuildUS500Params();
   MbFamilyStrategyManagePosition(trade,state,profile,snapshot,g_US500_last_atr_points,params,g_US500_last_position_modify);
  }

void EvaluateUS500Strategy(
   const MbRuntimeState &state,
   const MbSymbolProfile &profile,
   const MbMarketSnapshot &snapshot,
...
)
  {
   ...
   if(
      g_US500_ema_fast_handle == INVALID_HANDLE ||
      g_US500_ema_slow_handle == INVALID_HANDLE ||
      g_US500_atr_handle == INVALID_HANDLE ||
      g_US500_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_US500_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_US500_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_US500_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_US500_rsi_handle,rsi))
     {
  ...
  if(!MbStrategyResolveNewBar(profile.symbol,profile.trade_tf,g_US500_last_bar_time,current_bar_time,out.reason_code,MbShouldBypassFirstWaveTruthDiagnosticNewBar(profile.symbol,state.paper_mode_active)))
      return;
   ...
   double score_trend = (0.62 * ema_delta) + (0.14 * rsi_shift) + (0.24 * spread_bias);
   double score_pullback = (0.30 * rsi_shift) - (0.10 * ema_delta);
   double score_breakout = (0.82 * ema_delta) + (0.18 * spread_bias);
   double score_range = (-0.16 * ema_delta) - (0.12 * rsi_shift) + (0.28 * spread_bias);
   ...
   g_US500_last_atr_points = atr_raw / _Point;
   ...
   // US500 should stay brutally selective around the US cash rhythm and opening pressure.
   ...
   if(g_US500_tuning_policy.enabled && g_US500_tuning_policy.trusted_data)
     {
      int minutes_from_start = ResolveUS500MinutesFromWindowStart(profile);
      int minutes_to_end = ResolveUS500MinutesToWindowEnd(profile);

      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_US500_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_US500_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.breakout_conflict_tax);
         if(minutes_from_start >= 0 && minutes_from_start < 45)
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.index_opening_impulse_tax);
         if(minutes_to_end >= 0 && minutes_to_end < 45)
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.index_noon_transition_tax);
        }

      if(setup_reason == "SETUP_TREND" || setup_reason == "SETUP_PULLBACK")
        {
         if(g_US500_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TRENDLIKE_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.trend_no_aux_tax);
         if(g_US500_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_RANGE")
        {
         if(g_US500_tuning_policy.require_non_poor_candle_for_range && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_CANDLE";
           }
         if(g_US500_tuning_policy.require_non_poor_renko_for_range && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_RENKO";
           }
         if(g_US500_tuning_policy.require_support_for_rejection && !aux_supportive)
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_NEEDS_SUPPORT";
           }
         if(assessment.market_regime == "CHAOS")
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.range_chaos_tax);
         if(assessment.market_regime == "TREND" || assessment.market_regime == "BREAKOUT")
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.range_trend_tax);
         if(assessment.market_regime == "RANGE")
            ApplyUS500TuningBoost(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.rejection_range_boost);
         if(minutes_to_end >= 0 && minutes_to_end < 45)
            ApplyUS500TuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_US500_tuning_policy.index_noon_transition_tax);
         if(g_US500_tuning_policy.range_confidence_floor > 0.0 && fusion.confidence_score < g_US500_tuning_policy.range_confidence_floor)
           {
            fusion.allow_entry = false;
      ...
      fusion.confidence_score = MathMin(fusion.confidence_score,g_US500_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_US500_tuning_policy.risk_cap);
     }
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_US500_last_bar_time,out))
      return;
