...
void OnTimer()
  {
   ...
   if(MbShouldForceGlobalTeacherLearningTimerScan(g_profile.symbol,IsLocalPaperModeActive(),now))
     {
      AppendDE30DecisionEvent(now,"DIAGNOSTIC","FORCE","TIMER_FALLBACK_SCAN",g_market.spread_points,0.0,0.0,0,true,60);
   ...
   if(ShouldRunDE30TuningCycle(now))
     {
      ...
      MbApplyGlobalTeacherLearningTuningRescue(g_profile.symbol,IsLocalPaperModeActive(),g_DE30_family_tuning_policy,g_tuning_coordinator_state);
      ...
      if(MbPaperMaybeClosePosition(g_market,g_paper_position,now,paper_pnl,paper_close_reason,closed_paper))
        {
         ...
         AppendDE30DecisionEvent(now,"EXECUTION_TRUTH_CLOSE",(close_result.truth_close_written ? "OK" : "FAIL"),close_result.chain_reason,g_market.spread_points,0.0,0.0,0,false);
         AppendDE30DecisionEvent(now,"LESSON_WRITE",(close_result.lesson_written ? "OK" : "FAIL"),close_result.chain_reason,g_market.spread_points,0.0,0.0,0,false);
         AppendDE30DecisionEvent(now,"KNOWLEDGE_WRITE",(close_result.knowledge_bridge_enabled ? (close_result.knowledge_written ? "OK" : "FAIL") : "SKIP"),(close_result.knowledge_bridge_enabled ? close_result.chain_reason : "ML_BRIDGE_DISABLED"),g_market.spread_points,0.0,0.0,0,false);
      ...
      if(MbShouldBypassGlobalTeacherLearningRateGuard(g_profile.symbol,IsLocalPaperModeActive(),rate_guard.reason_code))
        {
         ...
         MbMicrobotHooksWriteSnapshot(g_hook_state,g_profile.symbol,IsLocalPaperModeActive(),g_paper_position,g_state,g_market,g_latency,g_ml_bridge);
         AppendDE30DecisionEvent(now,"RATE_GUARD","BYPASS",("PAPER_IGNORE_" + rate_guard.reason_code),g_market.spread_points,0.0,0.0,0,true,60);
        }
      else
        {
         g_state.halt = true;
      ...
      if(IsLocalPaperModeActive())
        {
         ...
         AppendDE30DecisionEvent(now,"EXECUTION_TRUTH_OPEN",(paper_open_result.truth_open_written ? "OK" : "FAIL"),(paper_open_result.truth_open_written ? paper_open_result.candidate_id : "PAPER_OPEN_TRUTH_WRITE_FAIL"),g_market.spread_points,signal.score,risk_plan.lots,0,false);
         AppendDE30DecisionEvent(now,"PAPER_POSITION_SAVE",(paper_open_result.paper_saved ? "OK" : "FAIL"),(paper_open_result.paper_saved ? (paper_opened ? paper_open_result.candidate_id : paper_open_result.reason_code) : "PAPER_POSITION_SAVE_FAIL"),g_market.spread_points,signal.score,risk_plan.lots,0,false);
         if(paper_opened)
           {
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
           ...
           {
            AppendDE30CandidateEvent(now,"PAPER_OPEN",false,paper_open_result.reason_code,signal,risk_plan.lots);
            AppendDE30DecisionEvent(now,"PAPER_OPEN","SKIP",paper_open_result.reason_code,g_market.spread_points,signal.score,risk_plan.lots,0,false);
           }
   ...
   if(trans.deal > 0)
     {
      ...
      if(live_close_result.live_close_processed)
        {
         AppendDE30DecisionEvent(now,"EXECUTION_TRUTH_CLOSE",(live_close_result.truth_close_written ? "OK" : "FAIL"),live_close_result.chain_reason,g_market.spread_points,0.0,deal_lots,0,false);
         AppendDE30DecisionEvent(now,"LESSON_WRITE",(live_close_result.lesson_written ? "OK" : "FAIL"),"LIVE_DEAL_CLOSE",g_market.spread_points,0.0,deal_lots,0,false);
        }
      AppendDE30DecisionEvent(now,"KNOWLEDGE_WRITE",(!live_close_result.live_close_processed ? "SKIP" : (live_close_result.knowledge_bridge_enabled ? (live_close_result.knowledge_written ? "OK" : "FAIL") : "SKIP")),(!live_close_result.live_close_processed ? "LIVE_DEAL_NOT_CLOSED" : (live_close_result.knowledge_bridge_enabled ? "LIVE_DEAL_CLOSE" : "ML_BRIDGE_DISABLED")),g_market.spread_points,0.0,deal_lots,0,false);
