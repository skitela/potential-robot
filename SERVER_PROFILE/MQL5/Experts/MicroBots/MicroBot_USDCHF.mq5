...
#property description "MicroBot USDCHF scaffold."
...
#include "..\\..\\Include\\Profiles\\Profile_USDCHF.mqh"
#include "..\\..\\Include\\Strategies\\Strategy_USDCHF.mqh"

input ulong InpMagic = 910106;
...
input string InpTradeComment = "MB_USDCHF";
input bool InpEnableStrategyTesterSandbox = true;
input string InpStrategyTesterSandboxTag = "USDCHF_AGENT";
...
MbTuningLocalPolicy g_usdchf_local_tuning_policy;
MbTuningLocalPolicy g_usdchf_effective_tuning_policy;
MbTuningFamilyPolicy g_usdchf_family_tuning_policy;
MbTuningCoordinatorState g_usdchf_coordinator_state;
...
bool ShouldRunUSDCHFTuningCycle(const datetime now)
  {
   const int tuning_service_interval_sec = 300;

   if(!g_usdchf_local_tuning_policy.enabled)
      return false;

   if(g_usdchf_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_usdchf_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_usdchf_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void ConfigureUSDCHFStrategyTesterSandbox()
  {
...
void AppendUSDCHFCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendUSDCHFAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   ...
   AppendUSDCHFDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }

double ApplyUSDCHFRiskMultiplierToLots(const MbMarketSnapshot &snapshot,const double base_lots,const double multiplier)
  {
...
int ResolveUSDCHFPaperHoldSeconds(const MbSignalDecision &signal)
  {
...
int OnInit()
  {
   ...
   LoadProfileUSDCHF(g_profile);
   if(!MbVerifyChartSymbol(g_profile.symbol))
      return(INIT_FAILED);
   g_profile.symbol = Symbol();
   ConfigureUSDCHFStrategyTesterSandbox();
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyUSDCHFInit(g_profile))
      return(INIT_FAILED);
   ...
   NormalizeUSDCHFMarketPermissions();
   ...
   MbTuningLocalPolicyReset(g_usdchf_local_tuning_policy);
   MbLoadTuningLocalPolicy(g_profile.symbol,g_usdchf_local_tuning_policy);
   MbTuningLocalPolicyReset(g_usdchf_effective_tuning_policy);
   MbTuningFamilyPolicyReset(g_usdchf_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_usdchf_coordinator_state);
   MbBuildEffectiveTuningPolicy(g_profile.session_profile,g_usdchf_local_tuning_policy,g_usdchf_effective_tuning_policy,g_usdchf_family_tuning_policy,g_usdchf_coordinator_state);
   StrategyUSDCHFSetTuningPolicy(g_usdchf_effective_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_usdchf_effective_tuning_policy);
   ...
   PrintFormat(
      "MB_USDCHF_ONNX_OBSERVATION enabled=%s ready=%s symbol=%s",
...
void OnDeinit(const int reason)
  {
   ...
   MbTesterTelemetryFinalizeSingleRun(g_profile,g_state,g_market,g_usdchf_effective_tuning_policy,g_latency);
   ...
   MbSaveTuningLocalPolicy(g_profile.symbol,g_usdchf_local_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_usdchf_effective_tuning_policy);
   StrategyUSDCHFDeinit();
...
void OnTimer()
  {
   ...
   if(MbShouldForceGlobalTeacherLearningTimerScan(g_profile.symbol,IsLocalPaperModeActive(),now))
     {
      AppendUSDCHFDecisionEvent(now,"DIAGNOSTIC","FORCE","TIMER_FALLBACK_SCAN",g_market.spread_points,0.0,0.0,0,true,60);
   ...
   MbFlushInformationalPolicy(g_profile,g_state,g_market,g_usdchf_local_tuning_policy,"BOOTSTRAP",(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbFlushBrokerProfile(g_profile,g_state,g_market);
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_usdchf_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbMlRuntimeBridgeFlushSnapshot(g_ml_bridge,g_profile,g_market,g_latency);
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   ...
   if(ShouldRunUSDCHFTuningCycle(now))
     {
      ...
      if(MbGetTuningFamilySymbols(g_profile.session_profile,family_symbols) > 0)
        {
         MbLoadTuningFamilyPolicy(g_profile.session_profile,g_usdchf_family_tuning_policy);
         family_changed = MbRunTuningFamilyAgent(g_profile.session_profile,family_symbols,g_usdchf_family_tuning_policy,family_reason);
        }

      if(MbGetAllTuningFamilies(families) > 0)
        {
         MbLoadTuningCoordinatorState(g_usdchf_coordinator_state);
         coordinator_changed = MbRunTuningCoordinator(families,g_usdchf_coordinator_state,coordinator_reason);
        }

      MbApplyGlobalTeacherLearningTuningRescue(g_profile.symbol,IsLocalPaperModeActive(),g_usdchf_family_tuning_policy,g_usdchf_coordinator_state);

      MbTuningDeckhandReport tuning_deckhand;
      MbTuningDeckhandReportReset(tuning_deckhand);
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_usdchf_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_usdchf_family_tuning_policy,g_usdchf_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_usdchf_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendUSDCHFDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_usdchf_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_usdchf_family_tuning_policy.freeze_new_changes)
         AppendUSDCHFDecisionEvent(now,"TUNING_FAMILY","SKIP",g_usdchf_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendUSDCHFDecisionEvent(now,"TUNING_FLEET","ADJUST",g_usdchf_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_usdchf_coordinator_state.freeze_new_changes)
         AppendUSDCHFDecisionEvent(now,"TUNING_FLEET","SKIP",g_usdchf_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendUSDCHFDecisionEvent(now,"TUNING","ADJUST",g_usdchf_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendUSDCHFDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_usdchf_local_tuning_policy.trusted_data)
         AppendUSDCHFDecisionEvent(now,"TUNING","SKIP",g_usdchf_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
...
double OnTester()
  {
   return MbTesterTelemetryOnTester(g_profile,g_state,g_market,g_usdchf_effective_tuning_policy,g_latency);
  }
...
void OnTick()
  {
      ...
      if(MbPaperMaybeClosePosition(g_market,g_paper_position,now,paper_pnl,paper_close_reason,closed_paper))
        {
         ...
         AppendUSDCHFDecisionEvent(now,"EXECUTION_TRUTH_CLOSE",(close_result.truth_close_written ? "OK" : "FAIL"),close_result.chain_reason,g_market.spread_points,0.0,0.0,0,false);
         AppendUSDCHFDecisionEvent(now,"LESSON_WRITE",(close_result.lesson_written ? "OK" : "FAIL"),close_result.chain_reason,g_market.spread_points,0.0,0.0,0,false);
         AppendUSDCHFDecisionEvent(now,"KNOWLEDGE_WRITE",(close_result.knowledge_bridge_enabled ? (close_result.knowledge_written ? "OK" : "FAIL") : "SKIP"),(close_result.knowledge_bridge_enabled ? close_result.chain_reason : "ML_BRIDGE_DISABLED"),g_market.spread_points,0.0,0.0,0,false);
         AppendUSDCHFDecisionEvent(now,"PAPER_CLOSE",(paper_pnl >= 0.0 ? "OK" : "LOSS"),paper_close_reason,g_market.spread_points,0.0,paper_pnl,0,false);
         MbSavePaperPosition(g_profile.symbol,g_paper_position);
   ...
   ManageUSDCHFOpenPosition(g_trade,g_state,g_profile,g_market);
      ...
      if(MbShouldBypassGlobalTeacherLearningRateGuard(g_profile.symbol,IsLocalPaperModeActive(),rate_guard.reason_code))
        {
         ...
         MbMicrobotHooksWriteSnapshot(g_hook_state,g_profile.symbol,IsLocalPaperModeActive(),g_paper_position,g_state,g_market,g_latency,g_ml_bridge);
         AppendUSDCHFDecisionEvent(now,"RATE_GUARD","BYPASS",("PAPER_IGNORE_" + rate_guard.reason_code),g_market.spread_points,0.0,0.0,0,true,60);
        }
      else
        {
         g_state.halt = true;
   ...
   if(already_has_position)
     {
      AppendUSDCHFDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,0.0,0.0,0,true,300);
   ...
   if(MbShouldBypassMarketGuardInPaper(IsLocalPaperModeActive(),guard_reason))
     {
      ...
      AppendUSDCHFDecisionEvent(now,"MARKET","BYPASS",("PAPER_IGNORE_" + guard_reason),g_market.spread_points,0.0,0.0,0,true,market_bypass_throttle);
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendUSDCHFDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendUSDCHFDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   MbRefreshPaperTradeRights(g_state,IsLocalPaperModeActive());
   EvaluateUSDCHFStrategy(g_state,g_profile,g_market,signal);
   ...
   if(signal.setup_type != "NONE")
      AppendUSDCHFAuxDecisionEvent(now,signal,(signal.score >= 0.0 ? MB_SIGNAL_BUY : MB_SIGNAL_SELL));
   ...
   if(IsLocalPaperModeActive() && !signal.valid && MbShouldBypassGlobalTeacherLearningSoftReject(g_profile.symbol,IsLocalPaperModeActive(),signal.setup_type,signal.reason_code))
     {
      ...
      bool blocked_by_usdchf_rejection_chaos_dirty_gate = false;
      if(signal.setup_type == "SETUP_TREND" && g_usdchf_effective_tuning_policy.require_non_poor_candle_for_trend && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_usdchf_effective_tuning_policy.require_non_poor_candle_for_breakout && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_usdchf_effective_tuning_policy.require_non_poor_renko_for_breakout && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_usdchf_effective_tuning_policy.require_non_poor_candle_for_range && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_usdchf_effective_tuning_policy.require_non_poor_renko_for_range && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_usdchf_effective_tuning_policy.range_confidence_floor > 0.0 && signal.confidence_score < g_usdchf_effective_tuning_policy.range_confidence_floor)
         blocked_by_tuning_gate = true;
      if(
      ...
      if(signal.setup_type == "SETUP_BREAKOUT" && poor_candle && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && signal.market_regime == "CHAOS" && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && signal.confidence_score < 0.75 && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT")
        {
          paper_gate_abs = 0.62;
      ...
      if(MbShouldRelaxGlobalTeacherLearningCostGate(g_profile.symbol,IsLocalPaperModeActive()))
        {
         blocked_by_usdchf_trend_chaos_dirty_gate = false;
         blocked_by_usdchf_rejection_chaos_dirty_gate = false;
        }

      if(
         !blocked_by_tuning_gate &&
         !blocked_by_usdchf_trend_chaos_dirty_gate &&
         !blocked_by_usdchf_rejection_chaos_dirty_gate &&
         (global_teacher_diag_active || MathAbs(signal.score) >= paper_gate_abs)
      )
        {
         signal.valid = true;
      ...
      else if(blocked_by_usdchf_trend_chaos_dirty_gate)
         signal.reason_code = "USDCHF_TREND_CHAOS_DIRTY_BLOCK";
      else if(blocked_by_usdchf_rejection_chaos_dirty_gate)
         signal.reason_code = "USDCHF_REJECTION_CHAOS_DIRTY_BLOCK";
     }
   ...
   AppendUSDCHFCandidateEvent(now,"EVALUATED",signal.valid,signal.reason_code,signal,0.0);
   if(!signal.valid)
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
   ...
   if(
      ...
      signal.setup_type == "SETUP_TREND" &&
      MathAbs(signal.score) >= 1.05 &&
      signal.candle_quality_grade != "POOR" &&
      signal.spread_regime != "BAD"
   )
     {
      risk_plan.allowed = true;
      risk_plan.reason_code = "PAPER_IGNORE_RISK_BLOCK_STRONG_SIGNAL";
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      ...
      AppendUSDCHFDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendUSDCHFCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendUSDCHFDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
      ...
      AppendUSDCHFDecisionEvent(now,"EXEC_PRECHECK","READY","PRECHECK_OK",g_market.spread_points,signal.score,risk_plan.lots,0,false);
      ...
      if(IsLocalPaperModeActive())
        {
         ...
         bool paper_opened = MbLearningOutcomeHandlePaperOpen(
            ...
            g_market,
            now,
            ResolveUSDCHFPaperHoldSeconds(signal),
            InpEnableLiveEntries,
            paper_open_result
         );
         AppendUSDCHFDecisionEvent(now,"EXECUTION_TRUTH_OPEN",(paper_open_result.truth_open_written ? "OK" : "FAIL"),(paper_open_result.truth_open_written ? paper_open_result.candidate_id : "PAPER_OPEN_TRUTH_WRITE_FAIL"),g_market.spread_points,signal.score,risk_plan.lots,0,false);
         AppendUSDCHFDecisionEvent(now,"PAPER_POSITION_SAVE",(paper_open_result.paper_saved ? "OK" : "FAIL"),(paper_open_result.paper_saved ? (paper_opened ? paper_open_result.candidate_id : paper_open_result.reason_code) : "PAPER_POSITION_SAVE_FAIL"),g_market.spread_points,signal.score,risk_plan.lots,0,false);
         if(paper_opened)
           {
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
            AppendUSDCHFCandidateEvent(now,"PAPER_OPEN",true,"PAPER_POSITION_OPENED",signal,risk_plan.lots);
            AppendUSDCHFDecisionEvent(now,"PAPER_OPEN","OK","PAPER_POSITION_OPENED",g_market.spread_points,signal.score,risk_plan.lots,0,false);
           }
         else
           {
            AppendUSDCHFCandidateEvent(now,"PAPER_OPEN",false,paper_open_result.reason_code,signal,risk_plan.lots);
            AppendUSDCHFDecisionEvent(now,"PAPER_OPEN","SKIP",paper_open_result.reason_code,g_market.spread_points,signal.score,risk_plan.lots,0,false);
           }
         ...
         MbMicrobotHooksWriteSnapshot(g_hook_state,g_profile.symbol,IsLocalPaperModeActive(),g_paper_position,g_state,g_market,g_latency,g_ml_bridge);
         AppendUSDCHFDecisionEvent(now,"EXEC_SEND","SKIP","LIVE_SEND_DISABLED",g_market.spread_points,signal.score,risk_plan.lots,0,true,30);
        }
      else
        {
         ...
         AppendUSDCHFCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendUSDCHFDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendUSDCHFDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
   ...
   if(trans.deal > 0)
     {
      ...
      if(live_close_result.live_close_processed)
        {
         AppendUSDCHFDecisionEvent(now,"EXECUTION_TRUTH_CLOSE",(live_close_result.truth_close_written ? "OK" : "FAIL"),live_close_result.chain_reason,g_market.spread_points,0.0,deal_lots,0,false);
         AppendUSDCHFDecisionEvent(now,"LESSON_WRITE",(live_close_result.lesson_written ? "OK" : "FAIL"),"LIVE_DEAL_CLOSE",g_market.spread_points,0.0,deal_lots,0,false);
        }
