...
function Get-Mt5DataDirs {
    param([string]$Server)
    $dirs = @()
    foreach ($dir in Get-AllTerminalDataDirs) {
        $ini = Join-Path $dir "config\common.ini"
...
foreach ($dir in $dirs) {
    ...
    if ($row.active_mq5_present) { $issues.Add("ACTIVE_HYBRID_AGENT_MQ5_PRESENT:" + $dir) }
    if ($row.active_ex5_present) { $issues.Add("ACTIVE_HYBRID_AGENT_EX5_PRESENT:" + $dir) }
    if (-not $row.detached_folder_present) { $issues.Add("DETACHED_FOLDER_MISSING:" + $dir) }
    $items += $row
}
...
$jsonPath = Join-Path $evidenceDir "validate_hybrid_agent_detached_on_vps_report.json"
$txtPath = Join-Path $evidenceDir "validate_hybrid_agent_detached_on_vps_report.txt"
