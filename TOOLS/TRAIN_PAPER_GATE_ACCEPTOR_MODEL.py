...
def markdown_report(metadata: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "## Model",
        f"- rodzina: `{metadata['model']['family']}`",
        f"- rola: `{metadata['model']['role']}`",
        f"- target treningowy: `{metadata['model']['training_target']}`",
        f"- zgodny z `ONNX`: `{str(metadata['model']['supports_onnx_export']).lower()}`",
        "",
        "## Dane",
...
def main() -> int:
    ...
    metadata: dict[str, Any] = {
        ...
        "model": {
            ...
            "role": ("MODEL_LOKALNY_PER_INSTRUMENT" if symbol_filter else "MODEL_GLOBALNY_BRAMKUJACY"),
            "training_target": "accepted",
