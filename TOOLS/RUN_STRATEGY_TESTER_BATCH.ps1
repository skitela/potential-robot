...
foreach ($run in $results) {
    $mdLines += ("- {0} / {1}: {2}, duration={3}, balance={4}, timeout_sec={5}, optimization_rows={6}" -f $run.symbol_alias, $run.sandbox_name, $run.result_label, $run.test_duration, $run.final_balance, $run.timeout_sec, $run.optimization_result_rows)
}
