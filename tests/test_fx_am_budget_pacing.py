...
class TestFxAmBudgetPacing(unittest.TestCase):
    ...
    def test_limit_curve(self) -> None:
        self.assertAlmostEqual(safetybot.fx_pacing_limit_ratio(0.10), 0.25, places=4)
        self.assertAlmostEqual(safetybot.fx_pacing_limit_ratio(0.50), 0.70, places=4)
        self.assertAlmostEqual(safetybot.fx_pacing_limit_ratio(0.90), 1.00, places=4)

    def test_allows_when_usage_within_early_limit(self) -> None:
        prev = (safetybot.CFG.fx_budget_pacing_enabled, safetybot.CFG.trade_windows)
        try:
            safetybot.CFG.fx_budget_pacing_enabled = True
            safetybot.CFG.trade_windows = {
                "FX_AM": {"group": "FX", "anchor_tz": "Europe/Warsaw", "start_hm": (9, 0), "end_hm": (12, 0)}
            }
            ok, meta = safetybot.fx_budget_pacing_allows_entry(
                _GovStub(cap=100),
                _DbStub(used=20),
                now_dt=self._pl_time_to_utc(9, 30),
            )
            self.assertTrue(ok)
            self.assertLessEqual(float(meta.get("used_ratio", 1.0)), 0.30)
        finally:
            (safetybot.CFG.fx_budget_pacing_enabled, safetybot.CFG.trade_windows) = prev
