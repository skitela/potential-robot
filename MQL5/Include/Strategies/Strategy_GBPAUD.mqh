#ifndef STRATEGY_GBPAUD_INCLUDED
#define STRATEGY_GBPAUD_INCLUDED
...
int g_gbpaud_ema_fast_handle = INVALID_HANDLE;
int g_gbpaud_ema_slow_handle = INVALID_HANDLE;
int g_gbpaud_atr_handle = INVALID_HANDLE;
int g_gbpaud_rsi_handle = INVALID_HANDLE;
datetime g_gbpaud_last_bar_time = 0;
double g_gbpaud_last_atr_points = 0.0;
datetime g_gbpaud_last_position_modify = 0;
MbTuningLocalPolicy g_gbpaud_tuning_policy;

struct GbpAudLocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildGBPAUDParams()
  {
   MbFamilyStrategyParams out;
   out.family_kind = MB_FAMILY_KIND_CROSS;
   out.ema_fast_period = 13;
   out.ema_slow_period = 36;
   out.atr_period = 14;
   out.rsi_period = 14;
   out.base_risk_pct = 0.29;
   out.execution_floor = 0.70;
   out.execution_decay = 0.50;
   out.min_risk_pct = 0.17;
   out.max_risk_pct = 0.42;
   out.sl_atr_multiplier = 1.90;
   out.sl_min_points = 44.0;
   out.tp_atr_multiplier = 2.40;
   out.tp_min_points = 72.0;
   out.trail_atr_multiplier = 1.32;
   out.pressure_step_scale = 6.0;
   out.ready_trigger_abs = 0.84;
   out.caution_trigger_abs = 1.00;
   return out;
  }

bool StrategyGBPAUDInit(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildGBPAUDParams();
   bool ok = MbFamilyStrategyInit(profile,params,g_gbpaud_ema_fast_handle,g_gbpaud_ema_slow_handle,g_gbpaud_atr_handle,g_gbpaud_rsi_handle,g_gbpaud_last_bar_time,g_gbpaud_last_atr_points,g_gbpaud_last_position_modify);
   MbTuningLocalPolicyReset(g_gbpaud_tuning_policy);
   return ok;
  }

void StrategyGBPAUDDeinit()
  {
   MbFamilyStrategyDeinit(g_gbpaud_ema_fast_handle,g_gbpaud_ema_slow_handle,g_gbpaud_atr_handle,g_gbpaud_rsi_handle,g_gbpaud_last_bar_time,g_gbpaud_last_atr_points,g_gbpaud_last_position_modify);
  }

void StrategyGBPAUDSetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_gbpaud_tuning_policy = policy;
  }

void ApplyGBPAUDTuningPenalty(
...
void BuildGBPAUDRiskPlan(
   const MbRuntimeState &state,
   const MbMarketSnapshot &snapshot,
   GbpAudLocalRiskPlan &out
)
  {
   MbFamilyStrategyParams params = BuildGBPAUDParams();
   MbFamilyStrategyBuildRiskPlan(state,snapshot,g_gbpaud_last_atr_points,params,out.allowed,out.lots,out.sl_points,out.tp_points,out.reason_code);
  }

void ManageGBPAUDOpenPosition(
   CTrade &trade,
  ...
  {
   MbFamilyStrategyParams params = BuildGBPAUDParams();
   MbFamilyStrategyManagePosition(trade,state,profile,snapshot,g_gbpaud_last_atr_points,params,g_gbpaud_last_position_modify);
  }

void EvaluateGBPAUDStrategy(
   const MbRuntimeState &state,
   const MbSymbolProfile &profile,
   const MbMarketSnapshot &snapshot,
...
)
  {
   ...
   if(
      g_gbpaud_ema_fast_handle == INVALID_HANDLE ||
      g_gbpaud_ema_slow_handle == INVALID_HANDLE ||
      g_gbpaud_atr_handle == INVALID_HANDLE ||
      g_gbpaud_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_gbpaud_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_gbpaud_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_gbpaud_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_gbpaud_rsi_handle,rsi))
     {
   ...
   if(!MbStrategyResolveNewBar(profile.symbol,profile.trade_tf,g_gbpaud_last_bar_time,current_bar_time,out.reason_code))
      return;
   ...
   g_gbpaud_last_atr_points = atr_raw / _Point;
   ...
   // GBPAUD cross-AUD flow is the noisiest of the AUD crosses and needs the strongest defensive filtering.
   ...
   if(setup_reason == "SETUP_BREAKOUT" && assessment.spread_regime == "BAD")
     {
      ...
      fusion.risk_multiplier = MathMax(0.55,fusion.risk_multiplier * 0.86);
      trigger_abs += 0.06;
     }
   ...
   if(setup_reason == "SETUP_BREAKOUT" && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN" || renko.reversal_flag))
     {
      fusion.confidence_score = MathMax(0.0,fusion.confidence_score - 0.06);
   ...
   if(setup_reason == "SETUP_RANGE" && assessment.spread_regime == "BAD")
     {
   ...
   if(setup_reason == "SETUP_BREAKOUT" && assessment.spread_regime == "BAD" && assessment.market_regime == "CHAOS")
     {
      fusion.confidence_score = MathMin(fusion.confidence_score,0.38);
      fusion.risk_multiplier = MathMax(0.55,MathMin(fusion.risk_multiplier,0.65));
      trigger_abs += 0.05;
     }
   if(setup_reason == "SETUP_TREND" && assessment.market_regime == "TREND" && assessment.spread_regime != "BAD")
     {
      if(candle.bias == renko.bias && candle.bias != "NONE" && candle.quality_grade != "POOR" && renko.quality_grade != "POOR")
        {
         fusion.confidence_score = MathMin(1.0,fusion.confidence_score + 0.04);
         fusion.risk_multiplier = MathMin(1.02,fusion.risk_multiplier * 1.02);
         trigger_abs = MathMax(0.20,trigger_abs - 0.02);
        }
     }

   if(g_gbpaud_tuning_policy.enabled && g_gbpaud_tuning_policy.trusted_data)
     {
      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_gbpaud_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_gbpaud_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.breakout_conflict_tax);
        }

      if(setup_reason == "SETUP_TREND" || setup_reason == "SETUP_PULLBACK")
        {
         if(g_gbpaud_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TRENDLIKE_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.trend_no_aux_tax);
         if(g_gbpaud_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_RANGE")
        {
         if(g_gbpaud_tuning_policy.require_non_poor_candle_for_range && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_CANDLE";
           }
         if(g_gbpaud_tuning_policy.require_non_poor_renko_for_range && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_RENKO";
           }
         if(g_gbpaud_tuning_policy.require_support_for_rejection && !aux_supportive)
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_NEEDS_SUPPORT";
           }
         if(assessment.market_regime == "CHAOS")
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.range_chaos_tax);
         if(assessment.market_regime == "TREND" || assessment.market_regime == "BREAKOUT")
            ApplyGBPAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.range_trend_tax);
         if(assessment.market_regime == "RANGE")
            ApplyGBPAUDTuningBoost(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpaud_tuning_policy.rejection_range_boost);
         if(g_gbpaud_tuning_policy.range_confidence_floor > 0.0 && fusion.confidence_score < g_gbpaud_tuning_policy.range_confidence_floor)
           {
            fusion.allow_entry = false;
      ...
      fusion.confidence_score = MathMin(fusion.confidence_score,g_gbpaud_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_gbpaud_tuning_policy.risk_cap);
     }
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_gbpaud_last_bar_time,out))
      return;
