#ifndef STRATEGY_PLATIN_INCLUDED
#define STRATEGY_PLATIN_INCLUDED
...
int g_PLATIN_ema_fast_handle = INVALID_HANDLE;
int g_PLATIN_ema_slow_handle = INVALID_HANDLE;
int g_PLATIN_atr_handle = INVALID_HANDLE;
int g_PLATIN_rsi_handle = INVALID_HANDLE;
datetime g_PLATIN_last_bar_time = 0;
double g_PLATIN_last_atr_points = 0.0;
datetime g_PLATIN_last_position_modify = 0;
MbTuningLocalPolicy g_PLATIN_tuning_policy;

struct PLATINLocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildPLATINParams()
  {
   MbFamilyStrategyParams out;
   out.family_kind = MB_FAMILY_KIND_METAL_FUTURES;
   out.ema_fast_period = 13;
   out.ema_slow_period = 37;
   out.atr_period = 14;
   out.rsi_period = 14;
   out.base_risk_pct = 0.18;
   out.execution_floor = 0.73;
   out.execution_decay = 0.50;
   out.min_risk_pct = 0.08;
   out.max_risk_pct = 0.24;
   out.sl_atr_multiplier = 2.20;
   out.sl_min_points = 220.0;
   out.tp_atr_multiplier = 2.80;
   out.tp_min_points = 330.0;
   out.trail_atr_multiplier = 1.22;
   out.pressure_step_scale = 8.0;
   out.ready_trigger_abs = 0.86;
   out.caution_trigger_abs = 1.04;
   return out;
  }

bool StrategyPLATINInit(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildPLATINParams();
   bool ok = MbFamilyStrategyInit(profile,params,g_PLATIN_ema_fast_handle,g_PLATIN_ema_slow_handle,g_PLATIN_atr_handle,g_PLATIN_rsi_handle,g_PLATIN_last_bar_time,g_PLATIN_last_atr_points,g_PLATIN_last_position_modify);
   MbTuningLocalPolicyReset(g_PLATIN_tuning_policy);
   return ok;
  }

void StrategyPLATINDeinit()
  {
   MbFamilyStrategyDeinit(g_PLATIN_ema_fast_handle,g_PLATIN_ema_slow_handle,g_PLATIN_atr_handle,g_PLATIN_rsi_handle,g_PLATIN_last_bar_time,g_PLATIN_last_atr_points,g_PLATIN_last_position_modify);
  }

void StrategyPLATINSetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_PLATIN_tuning_policy = policy;
  }

void ApplyPLATINTuningPenalty(
...
void BuildPLATINRiskPlan(
   const MbRuntimeState &state,
   const MbMarketSnapshot &snapshot,
   PLATINLocalRiskPlan &out
)
  {
   MbFamilyStrategyParams params = BuildPLATINParams();
   MbFamilyStrategyBuildRiskPlan(state,snapshot,g_PLATIN_last_atr_points,params,out.allowed,out.lots,out.sl_points,out.tp_points,out.reason_code);
  }

void ManagePLATINOpenPosition(
   CTrade &trade,
  ...
  {
   MbFamilyStrategyParams params = BuildPLATINParams();
   MbFamilyStrategyManagePosition(trade,state,profile,snapshot,g_PLATIN_last_atr_points,params,g_PLATIN_last_position_modify);
  }

void EvaluatePLATINStrategy(
   const MbRuntimeState &state,
   const MbSymbolProfile &profile,
   const MbMarketSnapshot &snapshot,
...
)
  {
   ...
   if(
      g_PLATIN_ema_fast_handle == INVALID_HANDLE ||
      g_PLATIN_ema_slow_handle == INVALID_HANDLE ||
      g_PLATIN_atr_handle == INVALID_HANDLE ||
      g_PLATIN_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_PLATIN_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_PLATIN_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_PLATIN_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_PLATIN_rsi_handle,rsi))
     {
   ...
   if(!MbStrategyResolveNewBar(profile.symbol,profile.trade_tf,g_PLATIN_last_bar_time,current_bar_time,out.reason_code))
      return;
   ...
   g_PLATIN_last_atr_points = atr_raw / _Point;
   ...
   // PLATIN behaves more like a directional futures metal than a calm spot precious flow.
   ...
   if(g_PLATIN_tuning_policy.enabled && g_PLATIN_tuning_policy.trusted_data)
     {
      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_PLATIN_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_PLATIN_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplyPLATINTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_PLATIN_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyPLATINTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_PLATIN_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplyPLATINTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_PLATIN_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplyPLATINTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_PLATIN_tuning_policy.breakout_conflict_tax);
        }

      if(setup_reason == "SETUP_TREND" || setup_reason == "SETUP_PULLBACK")
        {
         if(g_PLATIN_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TRENDLIKE_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplyPLATINTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_PLATIN_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyPLATINTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_PLATIN_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplyPLATINTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_PLATIN_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplyPLATINTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_PLATIN_tuning_policy.trend_no_aux_tax);
         if(g_PLATIN_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplyPLATINTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_RANGE")
        {
         if(g_PLATIN_tuning_policy.require_non_poor_candle_for_range && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_CANDLE";
           }
         if(g_PLATIN_tuning_policy.require_non_poor_renko_for_range && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_RENKO";
           }
         if(g_PLATIN_tuning_policy.range_confidence_floor > 0.0 && fusion.confidence_score < g_PLATIN_tuning_policy.range_confidence_floor)
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_CONFIDENCE_FLOOR";
           }
         if(g_PLATIN_tuning_policy.require_support_for_rejection && !aux_supportive)
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_NEEDS_SUPPORT";
           }
         if(assessment.market_regime == "RANGE")
            ApplyPLATINTuningBoost(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_PLATIN_tuning_policy.rejection_range_boost);
        }

      fusion.confidence_score = MathMin(fusion.confidence_score,g_PLATIN_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_PLATIN_tuning_policy.risk_cap);
     }
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_PLATIN_last_bar_time,out))
      return;
