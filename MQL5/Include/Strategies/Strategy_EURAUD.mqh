#ifndef STRATEGY_EURAUD_INCLUDED
#define STRATEGY_EURAUD_INCLUDED
...
int g_euraud_ema_fast_handle = INVALID_HANDLE;
int g_euraud_ema_slow_handle = INVALID_HANDLE;
int g_euraud_atr_handle = INVALID_HANDLE;
int g_euraud_rsi_handle = INVALID_HANDLE;
datetime g_euraud_last_bar_time = 0;
double g_euraud_last_atr_points = 0.0;
datetime g_euraud_last_position_modify = 0;
MbTuningLocalPolicy g_euraud_tuning_policy;

struct EurAudLocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildEURAUDParams()
  {
   MbFamilyStrategyParams out;
   out.family_kind = MB_FAMILY_KIND_CROSS;
   out.ema_fast_period = 14;
   out.ema_slow_period = 38;
   out.atr_period = 14;
   out.rsi_period = 14;
   out.base_risk_pct = 0.27;
   out.execution_floor = 0.72;
   out.execution_decay = 0.48;
   out.min_risk_pct = 0.16;
   out.max_risk_pct = 0.40;
   out.sl_atr_multiplier = 1.85;
   out.sl_min_points = 42.0;
   out.tp_atr_multiplier = 2.35;
   out.tp_min_points = 70.0;
   out.trail_atr_multiplier = 1.30;
   out.pressure_step_scale = 6.0;
   out.ready_trigger_abs = 0.83;
   out.caution_trigger_abs = 0.99;
   return out;
  }

bool StrategyEURAUDInit(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildEURAUDParams();
   bool ok = MbFamilyStrategyInit(profile,params,g_euraud_ema_fast_handle,g_euraud_ema_slow_handle,g_euraud_atr_handle,g_euraud_rsi_handle,g_euraud_last_bar_time,g_euraud_last_atr_points,g_euraud_last_position_modify);
   MbTuningLocalPolicyReset(g_euraud_tuning_policy);
   return ok;
  }

void StrategyEURAUDDeinit()
  {
   MbFamilyStrategyDeinit(g_euraud_ema_fast_handle,g_euraud_ema_slow_handle,g_euraud_atr_handle,g_euraud_rsi_handle,g_euraud_last_bar_time,g_euraud_last_atr_points,g_euraud_last_position_modify);
  }

void StrategyEURAUDSetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_euraud_tuning_policy = policy;
  }

void ApplyEURAUDTuningPenalty(
...
void BuildEURAUDRiskPlan(
   const MbRuntimeState &state,
   const MbMarketSnapshot &snapshot,
   EurAudLocalRiskPlan &out
)
  {
   MbFamilyStrategyParams params = BuildEURAUDParams();
   MbFamilyStrategyBuildRiskPlan(state,snapshot,g_euraud_last_atr_points,params,out.allowed,out.lots,out.sl_points,out.tp_points,out.reason_code);
  }

void ManageEURAUDOpenPosition(
   CTrade &trade,
  ...
  {
   MbFamilyStrategyParams params = BuildEURAUDParams();
   MbFamilyStrategyManagePosition(trade,state,profile,snapshot,g_euraud_last_atr_points,params,g_euraud_last_position_modify);
  }

void EvaluateEURAUDStrategy(
   const MbRuntimeState &state,
   const MbSymbolProfile &profile,
   const MbMarketSnapshot &snapshot,
...
)
  {
   ...
   if(
      g_euraud_ema_fast_handle == INVALID_HANDLE ||
      g_euraud_ema_slow_handle == INVALID_HANDLE ||
      g_euraud_atr_handle == INVALID_HANDLE ||
      g_euraud_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_euraud_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_euraud_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_euraud_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_euraud_rsi_handle,rsi))
     {
   ...
   if(!MbStrategyResolveNewBar(profile.symbol,profile.trade_tf,g_euraud_last_bar_time,current_bar_time,out.reason_code,MbShouldBypassGlobalTeacherLearningNewBar(profile.symbol,state.paper_mode_active)))
      return;
   ...
   g_euraud_last_atr_points = atr_raw / _Point;
   ...
   // EURAUD cross-AUD flow remains range-aware but needs stronger spread discipline than AUDUSD.
   ...
   if(g_euraud_tuning_policy.enabled && g_euraud_tuning_policy.trusted_data)
     {
      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_euraud_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_euraud_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.breakout_conflict_tax);
        }

      if(setup_reason == "SETUP_TREND" || setup_reason == "SETUP_PULLBACK")
        {
         if(g_euraud_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TRENDLIKE_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.trend_no_aux_tax);
         if(g_euraud_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_RANGE")
        {
         if(g_euraud_tuning_policy.require_non_poor_candle_for_range && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_CANDLE";
           }
         if(g_euraud_tuning_policy.require_non_poor_renko_for_range && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_RENKO";
           }
         if(g_euraud_tuning_policy.require_support_for_rejection && !aux_supportive)
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_NEEDS_SUPPORT";
           }
         if(assessment.market_regime == "CHAOS")
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.range_chaos_tax);
         if(assessment.market_regime == "TREND" || assessment.market_regime == "BREAKOUT")
            ApplyEURAUDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.range_trend_tax);
         if(assessment.market_regime == "RANGE")
            ApplyEURAUDTuningBoost(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_euraud_tuning_policy.rejection_range_boost);
         if(g_euraud_tuning_policy.range_confidence_floor > 0.0 && fusion.confidence_score < g_euraud_tuning_policy.range_confidence_floor)
           {
            fusion.allow_entry = false;
      ...
      fusion.confidence_score = MathMin(fusion.confidence_score,g_euraud_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_euraud_tuning_policy.risk_cap);
     }
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_euraud_last_bar_time,out))
      return;
