#ifndef STRATEGY_EURJPY_INCLUDED
#define STRATEGY_EURJPY_INCLUDED
...
int g_eurjpy_ema_fast_handle = INVALID_HANDLE;
int g_eurjpy_ema_slow_handle = INVALID_HANDLE;
int g_eurjpy_atr_handle = INVALID_HANDLE;
int g_eurjpy_rsi_handle = INVALID_HANDLE;
datetime g_eurjpy_last_bar_time = 0;
double g_eurjpy_last_atr_points = 0.0;
datetime g_eurjpy_last_position_modify = 0;
MbTuningLocalPolicy g_eurjpy_tuning_policy;

struct EurJpyLocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildEURJPYParams()
  {
   ...
   out.sl_atr_multiplier = 1.80;
   out.sl_min_points = 34.0;
   out.tp_atr_multiplier = 2.30;
   out.tp_min_points = 60.0;
   out.trail_atr_multiplier = 1.22;
...
bool StrategyEURJPYInit(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildEURJPYParams();
   bool ok = MbFamilyStrategyInit(profile,params,g_eurjpy_ema_fast_handle,g_eurjpy_ema_slow_handle,g_eurjpy_atr_handle,g_eurjpy_rsi_handle,g_eurjpy_last_bar_time,g_eurjpy_last_atr_points,g_eurjpy_last_position_modify);
   MbTuningLocalPolicyReset(g_eurjpy_tuning_policy);
   return ok;
  }

void StrategyEURJPYDeinit()
  {
   MbFamilyStrategyDeinit(g_eurjpy_ema_fast_handle,g_eurjpy_ema_slow_handle,g_eurjpy_atr_handle,g_eurjpy_rsi_handle,g_eurjpy_last_bar_time,g_eurjpy_last_atr_points,g_eurjpy_last_position_modify);
  }

void StrategyEURJPYSetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_eurjpy_tuning_policy = policy;
  }

void ApplyEURJPYTuningPenalty(
...
void BuildEURJPYRiskPlan(
   const MbRuntimeState &state,
   const MbMarketSnapshot &snapshot,
   EurJpyLocalRiskPlan &out
)
  {
   MbFamilyStrategyParams params = BuildEURJPYParams();
   MbFamilyStrategyBuildRiskPlan(state,snapshot,g_eurjpy_last_atr_points,params,out.allowed,out.lots,out.sl_points,out.tp_points,out.reason_code);
  }

void ManageEURJPYOpenPosition(
   CTrade &trade,
  ...
  {
   MbFamilyStrategyParams params = BuildEURJPYParams();
   MbFamilyStrategyManagePosition(trade,state,profile,snapshot,g_eurjpy_last_atr_points,params,g_eurjpy_last_position_modify);
  }

void EvaluateEURJPYStrategy(
   const MbRuntimeState &state,
   const MbSymbolProfile &profile,
   const MbMarketSnapshot &snapshot,
...
)
  {
   ...
   if(
      g_eurjpy_ema_fast_handle == INVALID_HANDLE ||
      g_eurjpy_ema_slow_handle == INVALID_HANDLE ||
      g_eurjpy_atr_handle == INVALID_HANDLE ||
      g_eurjpy_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_eurjpy_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_eurjpy_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_eurjpy_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_eurjpy_rsi_handle,rsi))
     {
   ...
   g_eurjpy_last_atr_points = atr_raw / _Point;
   ...
   // EURJPY cross flow should stay selective in noisy or rotational sessions.
   ...
   if(g_eurjpy_tuning_policy.enabled && g_eurjpy_tuning_policy.trusted_data)
     {
      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_eurjpy_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_eurjpy_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.breakout_conflict_tax);
        }

      if(setup_reason == "SETUP_TREND" || setup_reason == "SETUP_PULLBACK")
        {
         if(g_eurjpy_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TRENDLIKE_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.trend_no_aux_tax);
         if(g_eurjpy_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_RANGE")
        {
         if(g_eurjpy_tuning_policy.require_non_poor_candle_for_range && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_CANDLE";
           }
         if(g_eurjpy_tuning_policy.require_non_poor_renko_for_range && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_RENKO";
           }
         if(g_eurjpy_tuning_policy.require_support_for_rejection && !aux_supportive)
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_NEEDS_SUPPORT";
           }
         if(assessment.market_regime == "CHAOS")
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.range_chaos_tax);
         if(assessment.market_regime == "TREND" || assessment.market_regime == "BREAKOUT")
            ApplyEURJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.range_trend_tax);
         if(assessment.market_regime == "RANGE")
            ApplyEURJPYTuningBoost(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_eurjpy_tuning_policy.rejection_range_boost);
         if(g_eurjpy_tuning_policy.range_confidence_floor > 0.0 && fusion.confidence_score < g_eurjpy_tuning_policy.range_confidence_floor)
           {
            fusion.allow_entry = false;
      ...
      fusion.confidence_score = MathMin(fusion.confidence_score,g_eurjpy_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_eurjpy_tuning_policy.risk_cap);
     }
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_eurjpy_last_bar_time,out))
      return;
