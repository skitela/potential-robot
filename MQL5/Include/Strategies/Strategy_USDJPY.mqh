#ifndef STRATEGY_USDJPY_INCLUDED
#define STRATEGY_USDJPY_INCLUDED
...
int g_usdjpy_ema_fast_handle = INVALID_HANDLE;
int g_usdjpy_ema_slow_handle = INVALID_HANDLE;
int g_usdjpy_atr_handle = INVALID_HANDLE;
int g_usdjpy_rsi_handle = INVALID_HANDLE;
datetime g_usdjpy_last_bar_time = 0;
double g_usdjpy_last_atr_points = 0.0;
datetime g_usdjpy_last_position_modify = 0;
MbTuningLocalPolicy g_usdjpy_tuning_policy;

struct UsdJpyLocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildUSDJPYParams()
  {
   MbFamilyStrategyParams out;
   out.family_kind = MB_FAMILY_KIND_ASIA;
   out.ema_fast_period = 10;
   out.ema_slow_period = 28;
   out.atr_period = 14;
   out.rsi_period = 14;
   ...
   out.sl_atr_multiplier = 1.55;
   out.sl_min_points = 28.0;
   out.tp_atr_multiplier = 2.10;
   out.tp_min_points = 48.0;
   out.trail_atr_multiplier = 1.08;
   out.pressure_step_scale = 5.0;
   out.ready_trigger_abs = 0.80;
   out.caution_trigger_abs = 0.98;
   return out;
  }

bool StrategyUSDJPYInit(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildUSDJPYParams();
   bool ok = MbFamilyStrategyInit(profile,params,g_usdjpy_ema_fast_handle,g_usdjpy_ema_slow_handle,g_usdjpy_atr_handle,g_usdjpy_rsi_handle,g_usdjpy_last_bar_time,g_usdjpy_last_atr_points,g_usdjpy_last_position_modify);
   MbTuningLocalPolicyReset(g_usdjpy_tuning_policy);
   return ok;
  }

void StrategyUSDJPYDeinit()
  {
   MbFamilyStrategyDeinit(g_usdjpy_ema_fast_handle,g_usdjpy_ema_slow_handle,g_usdjpy_atr_handle,g_usdjpy_rsi_handle,g_usdjpy_last_bar_time,g_usdjpy_last_atr_points,g_usdjpy_last_position_modify);
  }

void StrategyUSDJPYSetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_usdjpy_tuning_policy = policy;
  }

void ApplyUSDJPYTuningPenalty(
...
void BuildUSDJPYRiskPlan(
   const MbRuntimeState &state,
   const MbMarketSnapshot &snapshot,
   UsdJpyLocalRiskPlan &out
)
  {
   MbFamilyStrategyParams params = BuildUSDJPYParams();
   MbFamilyStrategyBuildRiskPlan(state,snapshot,g_usdjpy_last_atr_points,params,out.allowed,out.lots,out.sl_points,out.tp_points,out.reason_code);
  }

void ManageUSDJPYOpenPosition(
   CTrade &trade,
  ...
  {
   MbFamilyStrategyParams params = BuildUSDJPYParams();
   MbFamilyStrategyManagePosition(trade,state,profile,snapshot,g_usdjpy_last_atr_points,params,g_usdjpy_last_position_modify);
  }

void EvaluateUSDJPYStrategy(
   const MbRuntimeState &state,
   const MbSymbolProfile &profile,
   const MbMarketSnapshot &snapshot,
...
)
  {
   ...
   if(
      g_usdjpy_ema_fast_handle == INVALID_HANDLE ||
      g_usdjpy_ema_slow_handle == INVALID_HANDLE ||
      g_usdjpy_atr_handle == INVALID_HANDLE ||
      g_usdjpy_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_usdjpy_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_usdjpy_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_usdjpy_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_usdjpy_rsi_handle,rsi))
     {
   ...
   g_usdjpy_last_atr_points = atr_raw / _Point;
   ...
   // JPY Asia flow should respect noisy sessions. Breakouts need stronger confirmation in CHAOS or RANGE.
   ...
   if(g_usdjpy_tuning_policy.enabled && g_usdjpy_tuning_policy.trusted_data)
     {
      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_usdjpy_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_usdjpy_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.breakout_conflict_tax);
        }

      if(setup_reason == "SETUP_TREND")
        {
         if(g_usdjpy_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TREND_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.trend_no_aux_tax);
         if(g_usdjpy_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_RANGE")
        {
         if(g_usdjpy_tuning_policy.require_non_poor_candle_for_range && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_CANDLE";
           }
         if(g_usdjpy_tuning_policy.require_non_poor_renko_for_range && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_RENKO";
           }
         if(assessment.market_regime == "CHAOS")
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.range_chaos_tax);
         if(assessment.market_regime == "TREND" || assessment.market_regime == "BREAKOUT")
            ApplyUSDJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_usdjpy_tuning_policy.range_trend_tax);
         if(g_usdjpy_tuning_policy.range_confidence_floor > 0.0 && fusion.confidence_score < g_usdjpy_tuning_policy.range_confidence_floor)
           {
            fusion.allow_entry = false;
      ...
      fusion.confidence_score = MathMin(fusion.confidence_score,g_usdjpy_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_usdjpy_tuning_policy.risk_cap);
     }
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_usdjpy_last_bar_time,out))
      return;
