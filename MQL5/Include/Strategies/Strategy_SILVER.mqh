#ifndef STRATEGY_SILVER_INCLUDED
#define STRATEGY_SILVER_INCLUDED
...
int g_SILVER_ema_fast_handle = INVALID_HANDLE;
int g_SILVER_ema_slow_handle = INVALID_HANDLE;
int g_SILVER_atr_handle = INVALID_HANDLE;
int g_SILVER_rsi_handle = INVALID_HANDLE;
datetime g_SILVER_last_bar_time = 0;
double g_SILVER_last_atr_points = 0.0;
datetime g_SILVER_last_position_modify = 0;
MbTuningLocalPolicy g_SILVER_tuning_policy;

struct SILVERLocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildSILVERParams()
  {
   ...
   out.atr_period = 14;
   out.rsi_period = 14;
   out.base_risk_pct = 0.20;
   out.execution_floor = 0.72;
   out.execution_decay = 0.50;
   out.min_risk_pct = 0.08;
   out.max_risk_pct = 0.26;
   out.sl_atr_multiplier = 2.35;
   out.sl_min_points = 260.0;
   out.tp_atr_multiplier = 2.95;
   out.tp_min_points = 390.0;
   out.trail_atr_multiplier = 1.26;
   out.pressure_step_scale = 9.0;
   out.ready_trigger_abs = 0.88;
   out.caution_trigger_abs = 1.06;
   return out;
  }

bool StrategySILVERInit(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildSILVERParams();
   bool ok = MbFamilyStrategyInit(
      profile,
      params,
      g_SILVER_ema_fast_handle,
      g_SILVER_ema_slow_handle,
      g_SILVER_atr_handle,
      g_SILVER_rsi_handle,
      g_SILVER_last_bar_time,
      g_SILVER_last_atr_points,
      g_SILVER_last_position_modify
   );
   MbTuningLocalPolicyReset(g_SILVER_tuning_policy);
   return ok;
  }

void StrategySILVERDeinit()
  {
...
void StrategySILVERSetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_SILVER_tuning_policy = policy;
  }

void ApplySILVERTuningPenalty(
...
void EvaluateSILVERStrategy(
...
)
  {
   ...
   if(
      g_SILVER_ema_fast_handle == INVALID_HANDLE ||
      g_SILVER_ema_slow_handle == INVALID_HANDLE ||
      g_SILVER_atr_handle == INVALID_HANDLE ||
      g_SILVER_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_SILVER_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_SILVER_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_SILVER_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_SILVER_rsi_handle,rsi))
     {
   ...
   g_SILVER_last_atr_points = atr_raw / _Point;
   ...
   // SILVER is noisier and more spread-sensitive than GOLD; weak structure should be punished quickly.
   ...
   if(assessment.spread_regime == "BAD")
     {
   ...
   if(setup_reason == "SETUP_REJECTION" && assessment.market_regime == "RANGE")
     {
      if(aux_strong_support && assessment.spread_regime != "BAD")
        {
   ...
   if(setup_reason == "SETUP_PULLBACK" && assessment.market_regime == "TREND" && aux_strong_support && assessment.spread_regime != "BAD")
     {
      fusion.confidence_score = MathMin(1.0,fusion.confidence_score + 0.02);
      fusion.risk_multiplier = MathMin(1.03,fusion.risk_multiplier * 1.02);
      trigger_abs = MathMax(0.22,trigger_abs - 0.02);
     }
   ...
   if(g_SILVER_tuning_policy.enabled && g_SILVER_tuning_policy.trusted_data)
     {
      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_SILVER_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_SILVER_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplySILVERTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_SILVER_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplySILVERTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_SILVER_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplySILVERTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_SILVER_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplySILVERTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_SILVER_tuning_policy.breakout_conflict_tax);
        }

      if(setup_reason == "SETUP_TREND")
        {
         if(g_SILVER_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TREND_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplySILVERTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_SILVER_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplySILVERTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_SILVER_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplySILVERTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_SILVER_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplySILVERTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_SILVER_tuning_policy.trend_no_aux_tax);
         if(g_SILVER_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplySILVERTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_REJECTION")
        {
         if(g_SILVER_tuning_policy.require_support_for_rejection && !aux_supportive)
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_REJECTION_NEEDS_SUPPORT";
           }
         if(assessment.market_regime == "RANGE")
            ApplySILVERTuningBoost(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_SILVER_tuning_policy.rejection_range_boost);
        }

      fusion.confidence_score = MathMin(fusion.confidence_score,g_SILVER_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_SILVER_tuning_policy.risk_cap);
     }
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_SILVER_last_bar_time,out))
      return;
