#ifndef STRATEGY_GBPUSD_INCLUDED
#define STRATEGY_GBPUSD_INCLUDED
...
int g_gbpusd_ema_fast_handle = INVALID_HANDLE;
int g_gbpusd_ema_slow_handle = INVALID_HANDLE;
int g_gbpusd_atr_handle = INVALID_HANDLE;
int g_gbpusd_rsi_handle = INVALID_HANDLE;
datetime g_gbpusd_last_bar_time = 0;
double g_gbpusd_last_atr_points = 0.0;
datetime g_gbpusd_last_position_modify = 0;
MbTuningLocalPolicy g_gbpusd_tuning_policy;

struct GbpUsdLocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildGBPUSDParams()
  {
   ...
   out.atr_period = 14;
   out.rsi_period = 14;
   out.base_risk_pct = 0.38;
   out.execution_floor = 0.68;
   out.execution_decay = 0.55;
   out.min_risk_pct = 0.22;
   out.max_risk_pct = 0.55;
   out.sl_atr_multiplier = 1.90;
   out.sl_min_points = 42.0;
   out.tp_atr_multiplier = 2.50;
   out.tp_min_points = 70.0;
   out.trail_atr_multiplier = 1.25;
   out.pressure_step_scale = 6.0;
   out.ready_trigger_abs = 0.76;
   out.caution_trigger_abs = 0.92;
   return out;
  }

bool StrategyGBPUSDInit(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildGBPUSDParams();
   bool ok = MbFamilyStrategyInit(
      profile,
      params,
      g_gbpusd_ema_fast_handle,
      g_gbpusd_ema_slow_handle,
      g_gbpusd_atr_handle,
      g_gbpusd_rsi_handle,
      g_gbpusd_last_bar_time,
      g_gbpusd_last_atr_points,
      g_gbpusd_last_position_modify
   );
   MbTuningLocalPolicyReset(g_gbpusd_tuning_policy);
   return ok;
  }

void StrategyGBPUSDDeinit()
  {
...
void StrategyGBPUSDSetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_gbpusd_tuning_policy = policy;
  }

void ApplyGBPUSDTuningPenalty(
...
void EvaluateGBPUSDStrategy(
...
)
  {
   ...
   if(
      g_gbpusd_ema_fast_handle == INVALID_HANDLE ||
      g_gbpusd_ema_slow_handle == INVALID_HANDLE ||
      g_gbpusd_atr_handle == INVALID_HANDLE ||
      g_gbpusd_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_gbpusd_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_gbpusd_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_gbpusd_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_gbpusd_rsi_handle,rsi))
     {
   ...
   if(!MbStrategyResolveNewBar(profile.symbol,profile.trade_tf,g_gbpusd_last_bar_time,current_bar_time,out.reason_code,MbShouldBypassGlobalTeacherLearningNewBar(profile.symbol,state.paper_mode_active)))
      return;
   ...
   g_gbpusd_last_atr_points = atr_raw / _Point;
   ...
   // GBPUSD follows the same family logic as EURUSD, but we keep the taxes slightly lighter.
   ...
   if(g_gbpusd_tuning_policy.enabled && g_gbpusd_tuning_policy.trusted_data)
     {
      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_gbpusd_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_gbpusd_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplyGBPUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpusd_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyGBPUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpusd_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplyGBPUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpusd_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplyGBPUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpusd_tuning_policy.breakout_conflict_tax);
        }

      if(setup_reason == "SETUP_TREND")
        {
         if(g_gbpusd_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TREND_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplyGBPUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpusd_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyGBPUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpusd_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplyGBPUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpusd_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplyGBPUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpusd_tuning_policy.trend_no_aux_tax);
         if(g_gbpusd_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplyGBPUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_REJECTION")
        {
         if(g_gbpusd_tuning_policy.require_support_for_rejection && !aux_supportive)
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_REJECTION_NEEDS_SUPPORT";
           }
         if(assessment.market_regime == "RANGE")
            ApplyGBPUSDTuningBoost(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpusd_tuning_policy.rejection_range_boost);
        }

      fusion.confidence_score = MathMin(fusion.confidence_score,g_gbpusd_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_gbpusd_tuning_policy.risk_cap);
     }
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_gbpusd_last_bar_time,out))
      return;
