#ifndef STRATEGY_NZDUSD_INCLUDED
#define STRATEGY_NZDUSD_INCLUDED
...
int g_nzdusd_ema_fast_handle = INVALID_HANDLE;
int g_nzdusd_ema_slow_handle = INVALID_HANDLE;
int g_nzdusd_atr_handle = INVALID_HANDLE;
int g_nzdusd_rsi_handle = INVALID_HANDLE;
datetime g_nzdusd_last_bar_time = 0;
double g_nzdusd_last_atr_points = 0.0;
datetime g_nzdusd_last_position_modify = 0;
MbTuningLocalPolicy g_nzdusd_tuning_policy;

struct NzdUsdLocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildNZDUSDParams()
  {
   ...
   out.atr_period = 14;
   out.rsi_period = 14;
   out.base_risk_pct = 0.28;
   out.execution_floor = 0.74;
   out.execution_decay = 0.50;
   out.min_risk_pct = 0.17;
   out.max_risk_pct = 0.42;
   out.sl_atr_multiplier = 1.60;
   out.sl_min_points = 24.0;
   out.tp_atr_multiplier = 2.00;
   out.tp_min_points = 42.0;
   out.trail_atr_multiplier = 1.14;
...
bool StrategyNZDUSDInit(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildNZDUSDParams();
   bool ok = MbFamilyStrategyInit(
      profile,
      params,
      g_nzdusd_ema_fast_handle,
      g_nzdusd_ema_slow_handle,
      g_nzdusd_atr_handle,
      g_nzdusd_rsi_handle,
      g_nzdusd_last_bar_time,
      g_nzdusd_last_atr_points,
      g_nzdusd_last_position_modify
   );
   MbTuningLocalPolicyReset(g_nzdusd_tuning_policy);
   return ok;
  }

void StrategyNZDUSDDeinit()
  {
...
void StrategyNZDUSDSetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_nzdusd_tuning_policy = policy;
  }

void ApplyNZDUSDTuningPenalty(
...
void EvaluateNZDUSDStrategy(
...
)
  {
   ...
   if(
      g_nzdusd_ema_fast_handle == INVALID_HANDLE ||
      g_nzdusd_ema_slow_handle == INVALID_HANDLE ||
      g_nzdusd_atr_handle == INVALID_HANDLE ||
      g_nzdusd_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_nzdusd_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_nzdusd_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_nzdusd_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_nzdusd_rsi_handle,rsi))
     {
   ...
   if(!MbStrategyResolveNewBar(profile.symbol,profile.trade_tf,g_nzdusd_last_bar_time,current_bar_time,out.reason_code))
      return;
   ...
   g_nzdusd_last_atr_points = atr_raw / _Point;
   ...
   if(setup_reason == "SETUP_RANGE" && assessment.market_regime == "CHAOS")
     {
      fusion.confidence_score = MathMax(0.0,fusion.confidence_score - 0.12);
      fusion.risk_multiplier = MathMax(0.55,fusion.risk_multiplier * 0.84);
      trigger_abs += 0.10;

      bool weak_range_chaos =
         (fusion.confidence_score < 0.58) ||
         !aux_supportive ||
         candle.quality_grade == "POOR" ||
         renko.quality_grade == "POOR" ||
         fusion.reason_code == "AUX_CONFLICT_CAUTION";
      if(weak_range_chaos)
        {
         fusion.allow_entry = false;
         fusion.reason_code = "NZDUSD_RANGE_CHAOS_BLOCK";
        }
     }
   if(setup_reason == "SETUP_RANGE" && assessment.market_regime == "TREND")
     {
      fusion.confidence_score = MathMax(0.0,fusion.confidence_score - 0.10);
      fusion.risk_multiplier = MathMax(0.55,fusion.risk_multiplier * 0.87);
      trigger_abs += 0.08;
     }
   if(setup_reason == "SETUP_RANGE" && assessment.spread_regime == "CAUTION")
     {
   ...
   if(setup_reason == "SETUP_RANGE" && candle.bias != "NONE" && renko.bias != "NONE" && candle.bias != renko.bias)
     {
      fusion.confidence_score = MathMax(0.0,fusion.confidence_score - 0.06);
   ...
   if(g_nzdusd_tuning_policy.enabled && g_nzdusd_tuning_policy.trusted_data)
     {
      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_nzdusd_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_nzdusd_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.breakout_conflict_tax);
        }

      if(setup_reason == "SETUP_TREND")
        {
         if(g_nzdusd_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TREND_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.trend_no_aux_tax);
         if(g_nzdusd_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_RANGE")
        {
         if(g_nzdusd_tuning_policy.require_non_poor_candle_for_range && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_CANDLE";
           }
         if(g_nzdusd_tuning_policy.require_non_poor_renko_for_range && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_RENKO";
           }
         if(assessment.market_regime == "CHAOS")
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.range_chaos_tax);
         if(assessment.market_regime == "TREND" || assessment.market_regime == "BREAKOUT")
            ApplyNZDUSDTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_nzdusd_tuning_policy.range_trend_tax);
         if(g_nzdusd_tuning_policy.range_confidence_floor > 0.0 && fusion.confidence_score < g_nzdusd_tuning_policy.range_confidence_floor)
           {
            fusion.allow_entry = false;
      ...
      fusion.confidence_score = MathMin(fusion.confidence_score,g_nzdusd_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_nzdusd_tuning_policy.risk_cap);
     }
   if(setup_reason == "SETUP_RANGE" && assessment.market_regime != "RANGE" && assessment.spread_regime == "CAUTION")
     {
      fusion.confidence_score = MathMax(0.0,fusion.confidence_score - 0.05);
      fusion.risk_multiplier = MathMax(0.55,fusion.risk_multiplier * 0.91);
      trigger_abs += 0.05;
     }
   if(state.loss_streak >= 8 || state.learning_bias <= -0.12)
     {
      fusion.confidence_score = MathMin(fusion.confidence_score,0.46);
      fusion.risk_multiplier = MathMax(0.55,MathMin(fusion.risk_multiplier,0.72));
      trigger_abs += 0.05;
     }
   if(state.loss_streak >= 15 || state.learning_bias <= -0.14)
     {
      fusion.confidence_score = MathMin(fusion.confidence_score,0.32);
      fusion.risk_multiplier = MathMax(0.55,MathMin(fusion.risk_multiplier,0.64));
      trigger_abs += 0.06;
     }
   if(candle.bias != "NONE" && renko.bias != "NONE" && candle.bias != renko.bias)
     {
      fusion.confidence_score = MathMax(0.0,fusion.confidence_score - 0.05);
      fusion.risk_multiplier = MathMax(0.55,fusion.risk_multiplier * 0.91);
      trigger_abs += 0.04;
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_nzdusd_last_bar_time,out))
      return;
