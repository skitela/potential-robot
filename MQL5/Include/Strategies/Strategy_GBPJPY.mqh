#ifndef STRATEGY_GBPJPY_INCLUDED
#define STRATEGY_GBPJPY_INCLUDED
...
int g_gbpjpy_ema_fast_handle = INVALID_HANDLE;
int g_gbpjpy_ema_slow_handle = INVALID_HANDLE;
int g_gbpjpy_atr_handle = INVALID_HANDLE;
int g_gbpjpy_rsi_handle = INVALID_HANDLE;
datetime g_gbpjpy_last_bar_time = 0;
double g_gbpjpy_last_atr_points = 0.0;
datetime g_gbpjpy_last_position_modify = 0;
MbTuningLocalPolicy g_gbpjpy_tuning_policy;

struct GbpJpyLocalRiskPlan
  {
   bool allowed;
   double lots;
   double sl_points;
...
MbFamilyStrategyParams BuildGBPJPYParams()
  {
   ...
   out.atr_period = 14;
   out.rsi_period = 14;
   out.base_risk_pct = 0.34;
   out.execution_floor = 0.70;
   out.execution_decay = 0.52;
   out.min_risk_pct = 0.20;
   out.max_risk_pct = 0.48;
   out.sl_atr_multiplier = 1.90;
   out.sl_min_points = 36.0;
   out.tp_atr_multiplier = 2.40;
   out.tp_min_points = 64.0;
   out.trail_atr_multiplier = 1.28;
...
bool StrategyGBPJPYInit(const MbSymbolProfile &profile)
  {
   MbFamilyStrategyParams params = BuildGBPJPYParams();
   bool ok = MbFamilyStrategyInit(profile,params,g_gbpjpy_ema_fast_handle,g_gbpjpy_ema_slow_handle,g_gbpjpy_atr_handle,g_gbpjpy_rsi_handle,g_gbpjpy_last_bar_time,g_gbpjpy_last_atr_points,g_gbpjpy_last_position_modify);
   MbTuningLocalPolicyReset(g_gbpjpy_tuning_policy);
   return ok;
  }

void StrategyGBPJPYDeinit()
  {
   MbFamilyStrategyDeinit(g_gbpjpy_ema_fast_handle,g_gbpjpy_ema_slow_handle,g_gbpjpy_atr_handle,g_gbpjpy_rsi_handle,g_gbpjpy_last_bar_time,g_gbpjpy_last_atr_points,g_gbpjpy_last_position_modify);
  }

void StrategyGBPJPYSetTuningPolicy(const MbTuningLocalPolicy &policy)
  {
   g_gbpjpy_tuning_policy = policy;
  }

void ApplyGBPJPYTuningPenalty(
...
void BuildGBPJPYRiskPlan(
   const MbRuntimeState &state,
   const MbMarketSnapshot &snapshot,
   GbpJpyLocalRiskPlan &out
)
  {
   MbFamilyStrategyParams params = BuildGBPJPYParams();
   MbFamilyStrategyBuildRiskPlan(state,snapshot,g_gbpjpy_last_atr_points,params,out.allowed,out.lots,out.sl_points,out.tp_points,out.reason_code);
  }

void ManageGBPJPYOpenPosition(
   CTrade &trade,
  ...
  {
   MbFamilyStrategyParams params = BuildGBPJPYParams();
   MbFamilyStrategyManagePosition(trade,state,profile,snapshot,g_gbpjpy_last_atr_points,params,g_gbpjpy_last_position_modify);
  }

void EvaluateGBPJPYStrategy(
   const MbRuntimeState &state,
   const MbSymbolProfile &profile,
   const MbMarketSnapshot &snapshot,
...
)
  {
   ...
   if(
      g_gbpjpy_ema_fast_handle == INVALID_HANDLE ||
      g_gbpjpy_ema_slow_handle == INVALID_HANDLE ||
      g_gbpjpy_atr_handle == INVALID_HANDLE ||
      g_gbpjpy_rsi_handle == INVALID_HANDLE
   )
     {
   ...
   double atr_raw = 0.0;
   double rsi = 50.0;
   if(!MbStrategyCopyLastValue(g_gbpjpy_ema_fast_handle,ema_fast))
     {
      out.reason_code = "EMA_FAST_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_gbpjpy_ema_slow_handle,ema_slow))
     {
      out.reason_code = "EMA_SLOW_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_gbpjpy_atr_handle,atr_raw))
     {
      out.reason_code = "ATR_COPY_FAIL";
      return;
     }
   if(!MbStrategyCopyLastValue(g_gbpjpy_rsi_handle,rsi))
     {
   ...
   if(!MbStrategyResolveNewBar(profile.symbol,profile.trade_tf,g_gbpjpy_last_bar_time,current_bar_time,out.reason_code))
      return;
   ...
   g_gbpjpy_last_atr_points = atr_raw / _Point;
   ...
   // GBPJPY cross flow should be the most selective among JPY crosses because of its volatility.
   ...
   if(setup_reason == "SETUP_RANGE" && (assessment.market_regime == "CHAOS" || assessment.spread_regime == "BAD"))
     {
      fusion.confidence_score = MathMax(0.0,fusion.confidence_score - 0.08);
      fusion.risk_multiplier = MathMax(0.55,fusion.risk_multiplier * 0.87);
      trigger_abs += 0.07;
     }
   if(setup_reason == "SETUP_RANGE" && (candle.quality_grade == "POOR" || renko.quality_grade == "POOR" || renko.reversal_flag))
     {
      fusion.confidence_score = MathMax(0.0,fusion.confidence_score - 0.05);
      fusion.risk_multiplier = MathMax(0.55,fusion.risk_multiplier * 0.90);
      trigger_abs += 0.04;
   ...
   if(g_gbpjpy_tuning_policy.enabled && g_gbpjpy_tuning_policy.trusted_data)
     {
      if(setup_reason == "SETUP_BREAKOUT")
        {
         if(g_gbpjpy_tuning_policy.require_non_poor_candle_for_breakout && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_CANDLE";
           }
         if(g_gbpjpy_tuning_policy.require_non_poor_renko_for_breakout && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_BREAKOUT_POOR_RENKO";
           }
         ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.breakout_global_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.breakout_chaos_tax);
         if(assessment.market_regime == "RANGE")
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.breakout_range_tax);
         if(fusion.reason_code == "AUX_CONFLICT_CAUTION")
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.breakout_conflict_tax);
        }

      if(setup_reason == "SETUP_TREND" || setup_reason == "SETUP_PULLBACK")
        {
         if(g_gbpjpy_tuning_policy.require_non_poor_candle_for_trend && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_TRENDLIKE_POOR_CANDLE";
           }
         if(assessment.market_regime == "BREAKOUT")
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.trend_breakout_tax);
         if(assessment.market_regime == "CHAOS")
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.trend_chaos_tax);
         if(assessment.spread_regime == "CAUTION" || assessment.spread_regime == "BAD")
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.trend_caution_tax);
         if(!aux_supportive)
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.trend_no_aux_tax);
         if(g_gbpjpy_tuning_policy.require_aux_support_for_trend && !aux_supportive)
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,0.04);
        }

      if(setup_reason == "SETUP_RANGE")
        {
         if(g_gbpjpy_tuning_policy.require_non_poor_candle_for_range && candle.quality_grade == "POOR")
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_CANDLE";
           }
         if(g_gbpjpy_tuning_policy.require_non_poor_renko_for_range && (renko.quality_grade == "POOR" || renko.quality_grade == "UNKNOWN"))
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_POOR_RENKO";
           }
         if(g_gbpjpy_tuning_policy.require_support_for_rejection && !aux_supportive)
           {
            fusion.allow_entry = false;
            fusion.reason_code = "TUNING_RANGE_NEEDS_SUPPORT";
           }
         if(assessment.market_regime == "CHAOS")
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.range_chaos_tax);
         if(assessment.market_regime == "TREND" || assessment.market_regime == "BREAKOUT")
            ApplyGBPJPYTuningPenalty(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.range_trend_tax);
         if(assessment.market_regime == "RANGE")
            ApplyGBPJPYTuningBoost(fusion.confidence_score,fusion.risk_multiplier,trigger_abs,g_gbpjpy_tuning_policy.rejection_range_boost);
         if(g_gbpjpy_tuning_policy.range_confidence_floor > 0.0 && fusion.confidence_score < g_gbpjpy_tuning_policy.range_confidence_floor)
           {
            fusion.allow_entry = false;
      ...
      fusion.confidence_score = MathMin(fusion.confidence_score,g_gbpjpy_tuning_policy.confidence_cap);
      fusion.risk_multiplier = MathMin(fusion.risk_multiplier,g_gbpjpy_tuning_policy.risk_cap);
     }
   ...
   if(!MbStrategyFinalizeSignalDecision(score,trigger_abs,setup_reason,current_bar_time,g_gbpjpy_last_bar_time,out))
      return;
