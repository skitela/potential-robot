...
#property description "MicroBot EURJPY scaffold."
...
#include "..\\..\\Include\\Profiles\\Profile_EURJPY.mqh"
#include "..\\..\\Include\\Strategies\\Strategy_EURJPY.mqh"

input ulong InpMagic = 910108;
...
input string InpTradeComment = "MB_EURJPY";
input bool InpEnableStrategyTesterSandbox = true;
input string InpStrategyTesterSandboxTag = "EURJPY_AGENT";
...
MbTuningLocalPolicy g_eurjpy_local_tuning_policy;
MbTuningLocalPolicy g_eurjpy_effective_tuning_policy;
MbTuningFamilyPolicy g_eurjpy_family_tuning_policy;
MbTuningCoordinatorState g_eurjpy_coordinator_state;
...
bool ShouldRunEURJPYTimerDiagnosticScan(const datetime now)
  {
...
bool ShouldRunEURJPYTuningCycle(const datetime now)
  {
   const int tuning_service_interval_sec = 300;

   if(!g_eurjpy_local_tuning_policy.enabled)
      return false;

   if(g_eurjpy_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_eurjpy_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_eurjpy_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void ConfigureEURJPYStrategyTesterSandbox()
  {
...
void EnsureEURJPYDecisionHeartbeat(const datetime ts,const MbOnnxObservationResult &onnx_result)
  {
...
void AppendEURJPYCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendEURJPYAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   ...
   AppendEURJPYDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }

double ApplyEURJPYRiskMultiplierToLots(const MbMarketSnapshot &snapshot,const double base_lots,const double multiplier)
  {
...
int ResolveEURJPaperHoldSeconds(const MbSignalDecision &signal)
  {
...
int OnInit()
  {
   ...
   LoadProfileEURJPY(g_profile);
   if(!MbVerifyChartSymbol(g_profile.symbol))
      return(INIT_FAILED);
   g_profile.symbol = Symbol();
   ConfigureEURJPYStrategyTesterSandbox();
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyEURJPYInit(g_profile))
      return(INIT_FAILED);
   ...
   NormalizeEURJPYMarketPermissions();
   ...
   MbTuningLocalPolicyReset(g_eurjpy_local_tuning_policy);
   MbLoadTuningLocalPolicy(g_profile.symbol,g_eurjpy_local_tuning_policy);
   MbTuningLocalPolicyReset(g_eurjpy_effective_tuning_policy);
   MbTuningFamilyPolicyReset(g_eurjpy_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_eurjpy_coordinator_state);
   MbBuildEffectiveTuningPolicy(g_profile.session_profile,g_eurjpy_local_tuning_policy,g_eurjpy_effective_tuning_policy,g_eurjpy_family_tuning_policy,g_eurjpy_coordinator_state);
   StrategyEURJPYSetTuningPolicy(g_eurjpy_effective_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_eurjpy_effective_tuning_policy);
   ...
   PrintFormat(
      "MB_EURJPY_ONNX_OBSERVATION enabled=%s ready=%s symbol=%s",
...
void OnDeinit(const int reason)
  {
   ...
   MbTesterTelemetryFinalizeSingleRun(g_profile,g_state,g_market,g_eurjpy_effective_tuning_policy,g_latency);
   ...
   MbSaveTuningLocalPolicy(g_profile.symbol,g_eurjpy_local_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_eurjpy_effective_tuning_policy);
   StrategyEURJPYDeinit();
...
void OnTimer()
  {
   ...
   if(ShouldRunEURJPYTimerDiagnosticScan(now))
     {
      g_last_timer_diagnostic_scan_ts = now;
      AppendEURJPYDecisionEvent(now,"DIAGNOSTIC","FORCE","TIMER_FALLBACK_SCAN",g_market.spread_points,0.0,0.0,0,true,60);
   ...
   EnsureEURJPYDecisionHeartbeat(now,timer_onnx_result);
   ...
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_eurjpy_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbMlRuntimeBridgeFlushSnapshot(g_ml_bridge,g_profile,g_market,g_latency);
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   ...
   if(ShouldRunEURJPYTuningCycle(now))
     {
      ...
      if(MbGetTuningFamilySymbolsForRuntime(g_profile.symbol,g_profile.session_profile,IsLocalPaperModeActive(),family_symbols) > 0)
        {
         MbLoadTuningFamilyPolicy(g_profile.session_profile,g_eurjpy_family_tuning_policy);
         family_changed = MbRunTuningFamilyAgent(g_profile.session_profile,family_symbols,g_eurjpy_family_tuning_policy,family_reason);
        }

      if(MbGetAllTuningFamilies(families) > 0)
        {
         MbLoadTuningCoordinatorState(g_eurjpy_coordinator_state);
         coordinator_changed = MbRunTuningCoordinator(families,g_eurjpy_coordinator_state,coordinator_reason);
        }

      MbTuningDeckhandReport tuning_deckhand;
      MbTuningDeckhandReportReset(tuning_deckhand);
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_eurjpy_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_eurjpy_family_tuning_policy,g_eurjpy_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_eurjpy_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendEURJPYDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_eurjpy_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_eurjpy_family_tuning_policy.freeze_new_changes)
         AppendEURJPYDecisionEvent(now,"TUNING_FAMILY","SKIP",g_eurjpy_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendEURJPYDecisionEvent(now,"TUNING_FLEET","ADJUST",g_eurjpy_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_eurjpy_coordinator_state.freeze_new_changes)
         AppendEURJPYDecisionEvent(now,"TUNING_FLEET","SKIP",g_eurjpy_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendEURJPYDecisionEvent(now,"TUNING","ADJUST",g_eurjpy_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendEURJPYDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_eurjpy_local_tuning_policy.trusted_data)
         AppendEURJPYDecisionEvent(now,"TUNING","SKIP",g_eurjpy_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
...
double OnTester()
  {
   return MbTesterTelemetryOnTester(g_profile,g_state,g_market,g_eurjpy_effective_tuning_policy,g_latency);
  }
...
void OnTick()
  {
   ...
   ManageEURJPYOpenPosition(g_trade,g_state,g_profile,g_market);
      ...
      if(MbShouldBypassFirstWaveTruthDiagnosticRateGuard(g_profile.symbol,IsLocalPaperModeActive(),rate_guard.reason_code))
        {
         ...
         AppendEURJPYDecisionEvent(now,"RATE_GUARD","BYPASS",("PAPER_IGNORE_" + rate_guard.reason_code),g_market.spread_points,0.0,0.0,0,true,60);
        }
      else
        {
         g_state.halt = true;
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendEURJPYDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
      return;
     }
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendEURJPYDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   MbRefreshPaperTradeRights(g_state,IsLocalPaperModeActive());
   EvaluateEURJPYStrategy(g_state,g_profile,g_market,signal);
   ...
   if(signal.setup_type != "NONE")
   ...
   else if(truth_diag_active)
     {
      ...
      AppendEURJPYDecisionEvent(now,"DIAGNOSTIC","SKIP",("NO_SETUP_" + no_setup_reason),g_market.spread_points,signal.score,0.0,0,true,60);
     }

   if(signal.setup_type != "NONE")
      AppendEURJPYAuxDecisionEvent(now,signal,(signal.score >= 0.0 ? MB_SIGNAL_BUY : MB_SIGNAL_SELL));
   ...
   if(IsLocalPaperModeActive() && soft_diag_reject)
     {
      ...
      bool blocked_by_eurjpy_range_chaos_bad_spread_gate = false;
      bool blocked_by_eurjpy_breakout_chaos_bad_spread_gate = false;
      bool blocked_by_eurjpy_trend_chaos_bad_spread_gate = false;
      if(signal.setup_type == "SETUP_TREND" && g_eurjpy_effective_tuning_policy.require_non_poor_candle_for_trend && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_eurjpy_effective_tuning_policy.require_non_poor_candle_for_breakout && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_eurjpy_effective_tuning_policy.require_non_poor_renko_for_breakout && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_eurjpy_effective_tuning_policy.require_non_poor_candle_for_range && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_eurjpy_effective_tuning_policy.require_non_poor_renko_for_range && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_eurjpy_effective_tuning_policy.range_confidence_floor > 0.0 && signal.confidence_score < g_eurjpy_effective_tuning_policy.range_confidence_floor)
         blocked_by_tuning_gate = true;
      if(
         ...
         blocked_by_eurjpy_range_chaos_bad_spread_gate = true;
      if(
         ...
         blocked_by_eurjpy_breakout_chaos_bad_spread_gate = true;
      if(
         ...
         blocked_by_eurjpy_trend_chaos_bad_spread_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT")
        {
         paper_gate_abs = 0.64;
      ...
      else if(signal.setup_type == "SETUP_RANGE")
        {
         ...
         if(signal.market_regime == "CHAOS" && signal.spread_regime == "BAD" && (weak_candle || poor_renko || signal.confidence_bucket == "LOW"))
            paper_gate_abs = MathMax(paper_gate_abs,0.42);
        }
      ...
      if(diagnostic_relaxes_cost_gate)
        {
         blocked_by_eurjpy_range_chaos_bad_spread_gate = false;
         blocked_by_eurjpy_breakout_chaos_bad_spread_gate = false;
         blocked_by_eurjpy_trend_chaos_bad_spread_gate = false;
        }
      ...
      if(
         ...
         !blocked_by_eurjpy_range_chaos_bad_spread_gate &&
         !blocked_by_eurjpy_breakout_chaos_bad_spread_gate &&
         !blocked_by_eurjpy_trend_chaos_bad_spread_gate &&
      ...
      else if(blocked_by_eurjpy_range_chaos_bad_spread_gate)
         signal.reason_code = "EURJPY_RANGE_CHAOS_BAD_SPREAD_BLOCK";
      else if(blocked_by_eurjpy_breakout_chaos_bad_spread_gate)
         signal.reason_code = "EURJPY_BREAKOUT_CHAOS_BAD_SPREAD_BLOCK";
   ...
   EurJpyLocalRiskPlan risk_plan;
   BuildEURJPYRiskPlan(g_state,g_market,risk_plan);
   ...
   if(signal.valid)
      risk_plan.lots = ApplyEURJPYRiskMultiplierToLots(g_market,risk_plan.lots,signal.risk_multiplier);
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      AppendEURJPYCandidateEvent(now,"SIZE_BLOCK",false,risk_plan.reason_code,signal,0.0);
      AppendEURJPYDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
      return;
     }
   ...
   if(signal.valid && (MbHasPosition(g_state.symbol,g_state.magic) || (IsLocalPaperModeActive() && MbPaperHasOpenPosition(g_paper_position))))
     {
      AppendEURJPYDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,signal.score,0.0,0,true,60);
      return;
     }
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendEURJPYCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendEURJPYDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         return;
        }
      ...
      if(IsLocalPaperModeActive())
         ...
         if(IsLocalPaperModeActive())
           {
            ...
            bool paper_opened = MbPaperOpenPosition(
               ...
               g_market.spread_points,
               now,
               ResolveEURJPaperHoldSeconds(signal),
               signal.reason_code,
               signal.setup_type,
      ...
      else
        {
         ...
         AppendEURJPYCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendEURJPYDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendEURJPYDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
  }
