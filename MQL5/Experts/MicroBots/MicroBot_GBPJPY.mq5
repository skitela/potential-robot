...
#property description "MicroBot GBPJPY scaffold."
...
#include "..\\..\\Include\\Profiles\\Profile_GBPJPY.mqh"
#include "..\\..\\Include\\Strategies\\Strategy_GBPJPY.mqh"

input ulong InpMagic = 910109;
input uint InpTimerSec = 5;
input bool InpEnableLiveEntries = false;
input bool InpPaperCollectMode = true;
input string InpTradeComment = "MB_GBPJPY";
input bool InpEnableStrategyTesterSandbox = true;
input string InpStrategyTesterSandboxTag = "GBPJPY_AGENT";
...
MbTuningLocalPolicy g_gbpjpy_local_tuning_policy;
MbTuningLocalPolicy g_gbpjpy_effective_tuning_policy;
MbTuningFamilyPolicy g_gbpjpy_family_tuning_policy;
MbTuningCoordinatorState g_gbpjpy_coordinator_state;
...
bool ShouldRunGBPJPYTuningCycle(const datetime now)
  {
   const int tuning_service_interval_sec = 300;

   if(!g_gbpjpy_local_tuning_policy.enabled)
      return false;

   if(g_gbpjpy_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_gbpjpy_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_gbpjpy_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void ConfigureGBPJPYStrategyTesterSandbox()
  {
...
void NormalizeGBPJPYMarketPermissions()
  {
...
void AppendGBPJPYDecisionEvent(
   const datetime ts,
   const string phase,
...
void AppendGBPJPYCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendGBPJPYAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   if(signal.setup_type == "NONE")
      return;
   ...
   AppendGBPJPYDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }

double ApplyGBPJPYRiskMultiplierToLots(const MbMarketSnapshot &snapshot,const double base_lots,const double multiplier)
  {
...
int ResolveGBPJPaperHoldSeconds(const MbSignalDecision &signal)
  {
...
int OnInit()
  {
   ...
   LoadProfileGBPJPY(g_profile);
   if(!MbVerifyChartSymbol(g_profile.symbol))
      return(INIT_FAILED);
   g_profile.symbol = Symbol();
   ConfigureGBPJPYStrategyTesterSandbox();
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyGBPJPYInit(g_profile))
      return(INIT_FAILED);
   ...
   NormalizeGBPJPYMarketPermissions();
   ...
   MbTuningLocalPolicyReset(g_gbpjpy_local_tuning_policy);
   MbLoadTuningLocalPolicy(g_profile.symbol,g_gbpjpy_local_tuning_policy);
   MbTuningLocalPolicyReset(g_gbpjpy_effective_tuning_policy);
   MbTuningFamilyPolicyReset(g_gbpjpy_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_gbpjpy_coordinator_state);
   MbBuildEffectiveTuningPolicy(g_profile.session_profile,g_gbpjpy_local_tuning_policy,g_gbpjpy_effective_tuning_policy,g_gbpjpy_family_tuning_policy,g_gbpjpy_coordinator_state);
   StrategyGBPJPYSetTuningPolicy(g_gbpjpy_effective_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_gbpjpy_effective_tuning_policy);
...
void OnDeinit(const int reason)
  {
   ...
   MbSaveTuningLocalPolicy(g_profile.symbol,g_gbpjpy_local_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_gbpjpy_effective_tuning_policy);
   StrategyGBPJPYDeinit();
...
void OnTimer()
  {
   ...
   MbFlushInformationalPolicy(g_profile,g_state,g_market,g_gbpjpy_local_tuning_policy,"BOOTSTRAP",(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbFlushBrokerProfile(g_profile,g_state,g_market);
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_gbpjpy_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   MbIncidentJournalFlush();
   ...
   if(ShouldRunGBPJPYTuningCycle(now))
     {
      ...
      if(MbGetTuningFamilySymbols(g_profile.session_profile,family_symbols) > 0)
        {
         MbLoadTuningFamilyPolicy(g_profile.session_profile,g_gbpjpy_family_tuning_policy);
         family_changed = MbRunTuningFamilyAgent(g_profile.session_profile,family_symbols,g_gbpjpy_family_tuning_policy,family_reason);
        }

      if(MbGetAllTuningFamilies(families) > 0)
        {
         MbLoadTuningCoordinatorState(g_gbpjpy_coordinator_state);
         coordinator_changed = MbRunTuningCoordinator(families,g_gbpjpy_coordinator_state,coordinator_reason);
        }

      MbTuningDeckhandReport tuning_deckhand;
      MbTuningDeckhandReportReset(tuning_deckhand);
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_gbpjpy_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_gbpjpy_family_tuning_policy,g_gbpjpy_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_gbpjpy_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendGBPJPYDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_gbpjpy_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_gbpjpy_family_tuning_policy.freeze_new_changes)
         AppendGBPJPYDecisionEvent(now,"TUNING_FAMILY","SKIP",g_gbpjpy_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendGBPJPYDecisionEvent(now,"TUNING_FLEET","ADJUST",g_gbpjpy_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_gbpjpy_coordinator_state.freeze_new_changes)
         AppendGBPJPYDecisionEvent(now,"TUNING_FLEET","SKIP",g_gbpjpy_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendGBPJPYDecisionEvent(now,"TUNING","ADJUST",g_gbpjpy_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendGBPJPYDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_gbpjpy_local_tuning_policy.trusted_data)
         AppendGBPJPYDecisionEvent(now,"TUNING","SKIP",g_gbpjpy_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
...
double OnTester()
  {
   return MbTesterTelemetryOnTester(g_profile,g_state,g_market,g_gbpjpy_effective_tuning_policy,g_latency);
  }
...
void OnTick()
  {
      ...
      if(MbPaperMaybeClosePosition(g_market,g_paper_position,now,paper_pnl,paper_close_reason,closed_paper))
        {
         ...
         AppendGBPJPYDecisionEvent(now,"PAPER_CLOSE",(paper_pnl >= 0.0 ? "OK" : "LOSS"),paper_close_reason,g_market.spread_points,closed_paper.confidence_score,paper_pnl,0,false);
         MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
        }
     }

   ManageGBPJPYOpenPosition(g_trade,g_state,g_profile,g_market);
   ...
   if(IsLocalPaperModeActive() && (guard_reason == "OUTSIDE_TRADE_WINDOW" || guard_reason == "MARGIN_FREE_LOW" || guard_reason == "SPREAD_CAP_EXCEEDED" || guard_reason == "TRADE_DISABLED"))
     {
      AppendGBPJPYDecisionEvent(now,"MARKET","BYPASS",("PAPER_IGNORE_" + guard_reason),g_market.spread_points,0.0,0.0,0,true,60);
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendGBPJPYDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
      return;
     }
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendGBPJPYDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   EvaluateGBPJPYStrategy(g_state,g_profile,g_market,signal);
   if(signal.setup_type != "NONE")
     {
   ...
   if(signal.setup_type != "NONE")
      AppendGBPJPYAuxDecisionEvent(now,signal,(signal.score >= 0.0 ? MB_SIGNAL_BUY : MB_SIGNAL_SELL));
   ...
   if(IsLocalPaperModeActive() && !signal.valid && signal.reason_code == "SCORE_BELOW_TRIGGER")
     {
      ...
      if(signal.setup_type == "SETUP_BREAKOUT" && g_gbpjpy_effective_tuning_policy.require_non_poor_candle_for_breakout && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_gbpjpy_effective_tuning_policy.require_non_poor_renko_for_breakout && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_gbpjpy_effective_tuning_policy.require_non_poor_candle_for_range && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_gbpjpy_effective_tuning_policy.require_non_poor_renko_for_range && poor_renko)
         blocked_by_tuning_gate = true;
   ...
   GbpJpyLocalRiskPlan risk_plan;
   BuildGBPJPYRiskPlan(g_state,g_market,risk_plan);
   ...
   if(signal.valid)
      risk_plan.lots = ApplyGBPJPYRiskMultiplierToLots(g_market,risk_plan.lots,signal.risk_multiplier);
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      AppendGBPJPYCandidateEvent(now,"SIZE_BLOCK",false,risk_plan.reason_code,signal,0.0);
      AppendGBPJPYDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
      return;
     }
   ...
   if(signal.valid && (MbHasPosition(g_state.symbol,g_state.magic) || (IsLocalPaperModeActive() && MbPaperHasOpenPosition(g_paper_position))))
     {
      AppendGBPJPYDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,signal.score,0.0,0,true,60);
      return;
     }
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendGBPJPYCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendGBPJPYDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         return;
        }

      AppendGBPJPYDecisionEvent(now,"EXEC_PRECHECK","READY","PRECHECK_OK",g_market.spread_points,signal.score,risk_plan.lots,0,false);
         ...
         if(IsLocalPaperModeActive())
           {
            ...
            MbPaperOpenPosition(
               ...
               g_market.spread_points,
               now,
               ResolveGBPJPaperHoldSeconds(signal),
               signal.reason_code,
               signal.setup_type,
            ...
            MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
            AppendGBPJPYCandidateEvent(now,"PAPER_OPEN",true,"PAPER_POSITION_OPENED",signal,risk_plan.lots);
            AppendGBPJPYDecisionEvent(now,"PAPER_OPEN","OK","PAPER_POSITION_OPENED",g_market.spread_points,signal.score,risk_plan.lots,0,false);
         }
         AppendGBPJPYDecisionEvent(now,"EXEC_SEND","SKIP","LIVE_SEND_DISABLED",g_market.spread_points,signal.score,risk_plan.lots,0,true,30);
        }
      else
        {
         MbMarkOrderSend(g_state);
         ...
         AppendGBPJPYCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendGBPJPYDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendGBPJPYDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
  }
