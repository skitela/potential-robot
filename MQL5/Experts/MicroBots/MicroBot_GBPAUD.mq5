...
#property description "MicroBot GBPAUD scaffold."
...
#include "..\\..\\Include\\Profiles\\Profile_GBPAUD.mqh"
#include "..\\..\\Include\\Strategies\\Strategy_GBPAUD.mqh"

input ulong InpMagic = 910111;
input uint InpTimerSec = 5;
input bool InpEnableLiveEntries = false;
input bool InpPaperCollectMode = true;
input string InpTradeComment = "MB_GBPAUD";
input bool InpEnableStrategyTesterSandbox = true;
input string InpStrategyTesterSandboxTag = "GBPAUD_AGENT";
...
MbTuningLocalPolicy g_gbpaud_local_tuning_policy;
MbTuningLocalPolicy g_gbpaud_effective_tuning_policy;
MbTuningFamilyPolicy g_gbpaud_family_tuning_policy;
MbTuningCoordinatorState g_gbpaud_coordinator_state;
...
bool ShouldRunGBPAUDTuningCycle(const datetime now)
  {
   const int tuning_service_interval_sec = 300;

   if(!g_gbpaud_local_tuning_policy.enabled)
      return false;

   if(g_gbpaud_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_gbpaud_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_gbpaud_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void ConfigureGBPAUDStrategyTesterSandbox()
  {
...
void AppendGBPAUDCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendGBPAUDAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   if(signal.setup_type == "NONE")
      return;
   ...
   AppendGBPAUDDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }

double ApplyGBPAUDRiskMultiplierToLots(const MbMarketSnapshot &snapshot,const double base_lots,const double multiplier)
  {
...
int ResolveGBPAUDPaperHoldSeconds(const MbSignalDecision &signal)
  {
...
int OnInit()
  {
   ...
   LoadProfileGBPAUD(g_profile);
   if(!MbVerifyChartSymbol(g_profile.symbol))
      return(INIT_FAILED);
   g_profile.symbol = Symbol();
   ConfigureGBPAUDStrategyTesterSandbox();
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyGBPAUDInit(g_profile))
      return(INIT_FAILED);
   ...
   MbTuningLocalPolicyReset(g_gbpaud_local_tuning_policy);
   MbLoadTuningLocalPolicy(g_profile.symbol,g_gbpaud_local_tuning_policy);
   MbTuningLocalPolicyReset(g_gbpaud_effective_tuning_policy);
   MbTuningFamilyPolicyReset(g_gbpaud_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_gbpaud_coordinator_state);
   MbBuildEffectiveTuningPolicy(g_profile.session_profile,g_gbpaud_local_tuning_policy,g_gbpaud_effective_tuning_policy,g_gbpaud_family_tuning_policy,g_gbpaud_coordinator_state);
   StrategyGBPAUDSetTuningPolicy(g_gbpaud_effective_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_gbpaud_effective_tuning_policy);
...
void OnDeinit(const int reason)
  {
   ...
   MbTesterTelemetryFinalizeSingleRun(g_profile,g_state,g_market,g_gbpaud_effective_tuning_policy,g_latency);
   ...
   MbSaveTuningLocalPolicy(g_profile.symbol,g_gbpaud_local_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_gbpaud_effective_tuning_policy);
   StrategyGBPAUDDeinit();
...
void OnTimer()
  {
   ...
   MbFlushInformationalPolicy(g_profile,g_state,g_market,g_gbpaud_local_tuning_policy,"BOOTSTRAP",(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbFlushBrokerProfile(g_profile,g_state,g_market);
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_gbpaud_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   MbIncidentJournalFlush();
   ...
   if(ShouldRunGBPAUDTuningCycle(now))
     {
      ...
      if(MbGetTuningFamilySymbols(g_profile.session_profile,family_symbols) > 0)
        {
         MbLoadTuningFamilyPolicy(g_profile.session_profile,g_gbpaud_family_tuning_policy);
         family_changed = MbRunTuningFamilyAgent(g_profile.session_profile,family_symbols,g_gbpaud_family_tuning_policy,family_reason);
        }

      if(MbGetAllTuningFamilies(families) > 0)
        {
         MbLoadTuningCoordinatorState(g_gbpaud_coordinator_state);
         coordinator_changed = MbRunTuningCoordinator(families,g_gbpaud_coordinator_state,coordinator_reason);
        }

      MbTuningDeckhandReport tuning_deckhand;
      MbTuningDeckhandReportReset(tuning_deckhand);
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_gbpaud_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_gbpaud_family_tuning_policy,g_gbpaud_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_gbpaud_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendGBPAUDDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_gbpaud_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_gbpaud_family_tuning_policy.freeze_new_changes)
         AppendGBPAUDDecisionEvent(now,"TUNING_FAMILY","SKIP",g_gbpaud_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendGBPAUDDecisionEvent(now,"TUNING_FLEET","ADJUST",g_gbpaud_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_gbpaud_coordinator_state.freeze_new_changes)
         AppendGBPAUDDecisionEvent(now,"TUNING_FLEET","SKIP",g_gbpaud_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendGBPAUDDecisionEvent(now,"TUNING","ADJUST",g_gbpaud_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendGBPAUDDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_gbpaud_local_tuning_policy.trusted_data)
         AppendGBPAUDDecisionEvent(now,"TUNING","SKIP",g_gbpaud_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
...
double OnTester()
  {
   return MbTesterTelemetryOnTester(g_profile,g_state,g_market,g_gbpaud_effective_tuning_policy,g_latency);
  }
...
void OnTick()
  {
      ...
      if(MbPaperMaybeClosePosition(g_market,g_paper_position,now,paper_pnl,paper_close_reason,closed_paper))
        {
         ...
         AppendGBPAUDDecisionEvent(now,"PAPER_CLOSE",(paper_pnl >= 0.0 ? "OK" : "LOSS"),paper_close_reason,g_market.spread_points,0.0,paper_pnl,0,false);
         MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
        }
     }

   ManageGBPAUDOpenPosition(g_trade,g_state,g_profile,g_market);
   ...
   if(already_has_position)
     {
      AppendGBPAUDDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,0.0,0.0,0,true,300);
   ...
   if(MbShouldBypassMarketGuardInPaper(IsLocalPaperModeActive(),guard_reason))
     {
      ...
      AppendGBPAUDDecisionEvent(now,"MARKET","BYPASS",("PAPER_IGNORE_" + guard_reason),g_market.spread_points,0.0,0.0,0,true,market_bypass_throttle);
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendGBPAUDDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
      return;
     }
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendGBPAUDDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   if(signal.setup_type != "NONE")
      AppendGBPAUDAuxDecisionEvent(now,signal,(signal.score >= 0.0 ? MB_SIGNAL_BUY : MB_SIGNAL_SELL));
   ...
   if(IsLocalPaperModeActive() && !signal.valid && signal.reason_code == "SCORE_BELOW_TRIGGER")
     {
      ...
      bool blocked_by_gbpaud_trend_chaos_bad_spread_gate = false;
      bool blocked_by_gbpaud_range_chaos_bad_spread_gate = false;
      if(signal.setup_type == "SETUP_TREND" && g_gbpaud_effective_tuning_policy.require_non_poor_candle_for_trend && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_gbpaud_effective_tuning_policy.require_non_poor_candle_for_breakout && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_gbpaud_effective_tuning_policy.require_non_poor_renko_for_breakout && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_gbpaud_effective_tuning_policy.require_non_poor_candle_for_range && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_gbpaud_effective_tuning_policy.require_non_poor_renko_for_range && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_gbpaud_effective_tuning_policy.range_confidence_floor > 0.0 && signal.confidence_score < g_gbpaud_effective_tuning_policy.range_confidence_floor)
         blocked_by_tuning_gate = true;
      if(
         ...
         blocked_by_gbpaud_trend_chaos_bad_spread_gate = true;
      if(
         ...
         blocked_by_gbpaud_range_chaos_bad_spread_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT")
        {
          paper_gate_abs = 0.62;
          if(signal.market_regime == "CHAOS" || signal.market_regime == "RANGE" || signal.confidence_bucket == "LOW")
             paper_gate_abs = 0.72;
          if(signal.spread_regime == "BAD" && (weak_candle || poor_renko))
      ...
      if(
         ...
         !blocked_by_gbpaud_trend_chaos_bad_spread_gate &&
         !blocked_by_gbpaud_range_chaos_bad_spread_gate &&
      ...
      else if(blocked_by_gbpaud_trend_chaos_bad_spread_gate)
         signal.reason_code = "GBPAUD_TREND_CHAOS_BAD_SPREAD_BLOCK";
      else if(blocked_by_gbpaud_range_chaos_bad_spread_gate)
         signal.reason_code = "GBPAUD_RANGE_CHAOS_BAD_SPREAD_BLOCK";
     }
   AppendGBPAUDCandidateEvent(now,"EVALUATED",signal.valid,signal.reason_code,signal,0.0);
   if(!signal.valid)
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);

   GbpAudLocalRiskPlan risk_plan;
   BuildGBPAUDRiskPlan(g_state,g_market,risk_plan);
   ...
   if(signal.valid)
      risk_plan.lots = ApplyGBPAUDRiskMultiplierToLots(g_market,risk_plan.lots,signal.risk_multiplier);
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      AppendGBPAUDCandidateEvent(now,"SIZE_BLOCK",false,risk_plan.reason_code,signal,0.0);
      AppendGBPAUDDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
      return;
     }
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendGBPAUDCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendGBPAUDDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         return;
        }
         ...
         if(IsLocalPaperModeActive())
           {
            ...
            MbPaperOpenPosition(
               ...
               g_market.spread_points,
               now,
               ResolveGBPAUDPaperHoldSeconds(signal),
               signal.reason_code,
               signal.setup_type,
            ...
            MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
            AppendGBPAUDCandidateEvent(now,"PAPER_OPEN",true,"PAPER_POSITION_OPENED",signal,risk_plan.lots);
            AppendGBPAUDDecisionEvent(now,"PAPER_OPEN","OK","PAPER_POSITION_OPENED",g_market.spread_points,signal.score,risk_plan.lots,0,false);
         }
         AppendGBPAUDDecisionEvent(now,"EXEC_SEND","SKIP","LIVE_SEND_DISABLED",g_market.spread_points,signal.score,risk_plan.lots,0,true,30);
        }
      else
        {
         MbMarkOrderSend(g_state);
         ...
         AppendGBPAUDCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendGBPAUDDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendGBPAUDDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
  }
