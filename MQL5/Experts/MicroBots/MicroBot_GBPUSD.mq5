...
#property description "MicroBot GBPUSD scaffold."
...
MbTuningLocalPolicy g_gbpusd_local_tuning_policy;
MbTuningLocalPolicy g_gbpusd_effective_tuning_policy;
MbTuningFamilyPolicy g_gbpusd_family_tuning_policy;
MbTuningCoordinatorState g_gbpusd_coordinator_state;
...
bool ShouldRunGBPUSDTuningCycle(const datetime now)
  {
   ...
   if(!g_gbpusd_local_tuning_policy.enabled)
      return false;

   if(g_gbpusd_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_gbpusd_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_gbpusd_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void ConfigureGBPUSDStrategyTesterSandbox()
  {
...
void AppendGBPUSDCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendGBPUSDAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   ...
   AppendGBPUSDDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }

double ApplyGBPUSDRiskMultiplierToLots(const MbMarketSnapshot &snapshot,const double base_lots,const double multiplier)
  {
...
int ResolveGBPUSDPaperHoldSeconds(const MbSignalDecision &signal)
  {
...
int OnInit()
  {
   ...
   LoadProfileGBPUSD(g_profile);
   if(!MbVerifyChartSymbol(g_profile.symbol))
      return(INIT_FAILED);
   g_profile.symbol = Symbol();
   ConfigureGBPUSDStrategyTesterSandbox();
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyGBPUSDInit(g_profile))
      return(INIT_FAILED);
   ...
   MbTuningLocalPolicyReset(g_gbpusd_local_tuning_policy);
   MbLoadTuningLocalPolicy(g_profile.symbol,g_gbpusd_local_tuning_policy);
   MbTuningLocalPolicyReset(g_gbpusd_effective_tuning_policy);
   MbTuningFamilyPolicyReset(g_gbpusd_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_gbpusd_coordinator_state);
...
void OnTimer()
  {
   ...
   if(MbShouldForceGlobalTeacherLearningTimerScan(g_profile.symbol,IsLocalPaperModeActive(),now))
     {
      AppendGBPUSDDecisionEvent(now,"DIAGNOSTIC","FORCE","TIMER_FALLBACK_SCAN",g_market.spread_points,0.0,0.0,0,true,60);
   ...
   MbFlushInformationalPolicy(g_profile,g_state,g_market,g_gbpusd_local_tuning_policy,"BOOTSTRAP",(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbFlushBrokerProfile(g_profile,g_state,g_market);
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_gbpusd_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbMlRuntimeBridgeFlushSnapshot(g_ml_bridge,g_profile,g_market,g_latency);
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   ...
   if(ShouldRunGBPUSDTuningCycle(now))
     {
      ...
      if(MbGetTuningFamilySymbols(g_profile.session_profile,family_symbols) > 0)
        {
         MbLoadTuningFamilyPolicy(g_profile.session_profile,g_gbpusd_family_tuning_policy);
         family_changed = MbRunTuningFamilyAgent(g_profile.session_profile,family_symbols,g_gbpusd_family_tuning_policy,family_reason);
        }

      if(MbGetAllTuningFamilies(families) > 0)
        {
         MbLoadTuningCoordinatorState(g_gbpusd_coordinator_state);
         coordinator_changed = MbRunTuningCoordinator(families,g_gbpusd_coordinator_state,coordinator_reason);
        }

      MbApplyGlobalTeacherLearningTuningRescue(g_profile.symbol,IsLocalPaperModeActive(),g_gbpusd_family_tuning_policy,g_gbpusd_coordinator_state);

      MbTuningDeckhandReport tuning_deckhand;
      MbTuningDeckhandReportReset(tuning_deckhand);
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_gbpusd_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_gbpusd_family_tuning_policy,g_gbpusd_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_gbpusd_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendGBPUSDDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_gbpusd_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_gbpusd_family_tuning_policy.freeze_new_changes)
         AppendGBPUSDDecisionEvent(now,"TUNING_FAMILY","SKIP",g_gbpusd_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendGBPUSDDecisionEvent(now,"TUNING_FLEET","ADJUST",g_gbpusd_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_gbpusd_coordinator_state.freeze_new_changes)
         AppendGBPUSDDecisionEvent(now,"TUNING_FLEET","SKIP",g_gbpusd_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendGBPUSDDecisionEvent(now,"TUNING","ADJUST",g_gbpusd_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendGBPUSDDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_gbpusd_local_tuning_policy.trusted_data)
         AppendGBPUSDDecisionEvent(now,"TUNING","SKIP",g_gbpusd_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
...
double OnTester()
  {
   return MbTesterTelemetryOnTester(g_profile,g_state,g_market,g_gbpusd_effective_tuning_policy,g_latency);
  }
...
void OnTick()
  {
      ...
      if(MbPaperMaybeClosePosition(g_market,g_paper_position,now,paper_pnl,paper_close_reason,closed_paper))
        {
         ...
         AppendGBPUSDDecisionEvent(now,"EXECUTION_TRUTH_CLOSE",(close_result.truth_close_written ? "OK" : "FAIL"),close_result.chain_reason,g_market.spread_points,0.0,0.0,0,false);
         AppendGBPUSDDecisionEvent(now,"LESSON_WRITE",(close_result.lesson_written ? "OK" : "FAIL"),close_result.chain_reason,g_market.spread_points,0.0,0.0,0,false);
         AppendGBPUSDDecisionEvent(now,"KNOWLEDGE_WRITE",(close_result.knowledge_bridge_enabled ? (close_result.knowledge_written ? "OK" : "FAIL") : "SKIP"),(close_result.knowledge_bridge_enabled ? close_result.chain_reason : "ML_BRIDGE_DISABLED"),g_market.spread_points,0.0,0.0,0,false);
         AppendGBPUSDDecisionEvent(now,"PAPER_CLOSE",(paper_pnl >= 0.0 ? "OK" : "LOSS"),paper_close_reason,g_market.spread_points,0.0,paper_pnl,0,false);
         MbSavePaperPosition(g_profile.symbol,g_paper_position);
   ...
   ManageGBPUSDOpenPosition(g_trade,g_state,g_profile,g_market);
   ...
   if(rate_guard.halt)
     {
      if(IsLocalPaperModeActive() && rate_guard.reason_code == "BROKER_PRICE_RATE_LIMIT")
        {
         g_state.halt = false;
         g_state.caution_mode = true;
         AppendGBPUSDDecisionEvent(now,"RATE_GUARD","BYPASS","PAPER_IGNORE_BROKER_PRICE_RATE_LIMIT",g_market.spread_points,0.0,0.0,0,true,60);
        }
      else
        {
         g_state.halt = true;
   ...
   if(already_has_position)
     {
      AppendGBPUSDDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,0.0,0.0,0,true,300);
   ...
   if(MbShouldBypassMarketGuardInPaper(IsLocalPaperModeActive(),guard_reason))
     {
      ...
      AppendGBPUSDDecisionEvent(now,"MARKET","BYPASS",("PAPER_IGNORE_" + guard_reason),g_market.spread_points,0.0,0.0,0,true,market_bypass_throttle);
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendGBPUSDDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendGBPUSDDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   MbRefreshPaperTradeRights(g_state,IsLocalPaperModeActive());
   EvaluateGBPUSDStrategy(g_state,g_profile,g_market,signal);
   ...
   if(signal.setup_type != "NONE")
      AppendGBPUSDAuxDecisionEvent(now,signal,(signal.score >= 0.0 ? MB_SIGNAL_BUY : MB_SIGNAL_SELL));
   ...
   if(IsLocalPaperModeActive() && !signal.valid && MbShouldBypassGlobalTeacherLearningSoftReject(g_profile.symbol,IsLocalPaperModeActive(),signal.setup_type,signal.reason_code))
     {
      ...
      if(signal.setup_type == "SETUP_BREAKOUT" && g_gbpusd_effective_tuning_policy.require_non_poor_candle_for_breakout && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_gbpusd_effective_tuning_policy.require_non_poor_renko_for_breakout && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && signal.market_regime == "CHAOS" && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && signal.confidence_bucket == "LOW" && (poor_candle || poor_renko))
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && signal.spread_regime == "BAD" && poor_candle)
         blocked_by_tuning_gate = true;
      if(
      ...
      if(signal.setup_type == "SETUP_BREAKOUT")
      ...
      else if(signal.setup_type == "SETUP_PULLBACK")
        {
         if(signal.market_regime == "CHAOS" && signal.confidence_bucket == "LOW" && poor_candle && poor_renko)
            paper_gate_abs = 0.34;
         else if(signal.market_regime == "CHAOS" && poor_candle)
            paper_gate_abs = 0.28;
         else if(signal.market_regime == "TREND" && !poor_candle && !poor_renko)
            paper_gate_abs = 0.24;
        }
      else if(signal.setup_type == "SETUP_REJECTION")
        {
         ...
         if(poor_candle && signal.confidence_bucket != "HIGH")
      ...
      if(MbShouldRelaxGlobalTeacherLearningCostGate(g_profile.symbol,IsLocalPaperModeActive()))
        {
         blocked_by_gbpusd_rejection_poor_candle_gate = false;
         blocked_by_gbpusd_trend_chaos_bad_spread_gate = false;
         blocked_by_gbpusd_breakout_chaos_bad_spread_gate = false;
        }
   ...
   GbpUsdLocalRiskPlan risk_plan;
   BuildGBPUSDRiskPlan(g_state,g_market,risk_plan);
   ...
   if(
      ...
      (signal.setup_type == "SETUP_TREND" || signal.setup_type == "SETUP_PULLBACK") &&
      MathAbs(signal.score) >= 0.95 &&
      signal.candle_quality_grade != "POOR" &&
      signal.renko_quality_grade != "POOR" &&
      signal.spread_regime != "BAD"
   )
   ...
   if(
      ...
      risk_plan.reason_code == "RISK_CONTRACT_BLOCK" &&
      signal.setup_type == "SETUP_BREAKOUT" &&
      MathAbs(signal.score) >= 1.00 &&
      signal.market_regime != "CHAOS" &&
      signal.spread_regime != "BAD" &&
      signal.execution_regime == "GOOD" &&
      signal.confidence_bucket != "LOW" &&
      (
         signal.candle_quality_grade == "GOOD" ||
         signal.renko_quality_grade == "GOOD" ||
         (
            (signal.candle_quality_grade == "FAIR" || signal.candle_quality_grade == "GOOD") &&
            (signal.renko_quality_grade == "FAIR" || signal.renko_quality_grade == "GOOD")
         )
      )
     )
     {
      risk_plan.allowed = true;
      risk_plan.reason_code = "PAPER_IGNORE_RISK_BLOCK_CLEAN_BREAKOUT";
      risk_plan.lots = MathMax(g_market.vol_min,g_market.vol_step);
     }
   if(
      IsLocalPaperModeActive() &&
      ...
      risk_plan.reason_code == "RISK_CONTRACT_BLOCK" &&
      signal.setup_type == "SETUP_TREND" &&
      signal.side == MB_SIGNAL_SELL &&
      MathAbs(signal.score) >= (signal.market_regime == "CHAOS" ? 0.92 : 0.78) &&
      signal.execution_regime == "GOOD" &&
      signal.spread_regime != "BAD" &&
      signal.renko_bias == "DOWN" &&
      signal.renko_quality_grade != "POOR" &&
      signal.candle_quality_grade != "UNKNOWN"
   )
     {
      risk_plan.allowed = true;
      risk_plan.reason_code = "PAPER_IGNORE_RISK_BLOCK_TREND_SELL";
      risk_plan.lots = MathMax(g_market.vol_min,g_market.vol_step);
     }
   if(signal.valid)
      risk_plan.lots = ApplyGBPUSDRiskMultiplierToLots(g_market,risk_plan.lots,signal.risk_multiplier);
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      AppendGBPUSDCandidateEvent(now,"SIZE_BLOCK",false,risk_plan.reason_code,signal,0.0);
      AppendGBPUSDDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendGBPUSDCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendGBPUSDDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
      ...
      AppendGBPUSDDecisionEvent(now,"EXEC_PRECHECK","READY","PRECHECK_OK",g_market.spread_points,signal.score,risk_plan.lots,0,false);
      ...
      if(IsLocalPaperModeActive())
        {
         ...
         bool paper_opened = MbLearningOutcomeHandlePaperOpen(
            ...
            g_market,
            now,
            ResolveGBPUSDPaperHoldSeconds(signal),
            InpEnableLiveEntries,
            paper_open_result
         );
         AppendGBPUSDDecisionEvent(now,"EXECUTION_TRUTH_OPEN",(paper_open_result.truth_open_written ? "OK" : "FAIL"),(paper_open_result.truth_open_written ? paper_open_result.candidate_id : "PAPER_OPEN_TRUTH_WRITE_FAIL"),g_market.spread_points,signal.score,risk_plan.lots,0,false);
         AppendGBPUSDDecisionEvent(now,"PAPER_POSITION_SAVE",(paper_open_result.paper_saved ? "OK" : "FAIL"),(paper_open_result.paper_saved ? (paper_opened ? paper_open_result.candidate_id : paper_open_result.reason_code) : "PAPER_POSITION_SAVE_FAIL"),g_market.spread_points,signal.score,risk_plan.lots,0,false);
         if(paper_opened)
           {
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
            AppendGBPUSDCandidateEvent(now,"PAPER_OPEN",true,"PAPER_POSITION_OPENED",signal,risk_plan.lots);
            AppendGBPUSDDecisionEvent(now,"PAPER_OPEN","OK","PAPER_POSITION_OPENED",g_market.spread_points,signal.score,risk_plan.lots,0,false);
           }
         else
           {
            AppendGBPUSDCandidateEvent(now,"PAPER_OPEN",false,paper_open_result.reason_code,signal,risk_plan.lots);
            AppendGBPUSDDecisionEvent(now,"PAPER_OPEN","SKIP",paper_open_result.reason_code,g_market.spread_points,signal.score,risk_plan.lots,0,false);
           }
         ...
         MbMicrobotHooksWriteSnapshot(g_hook_state,g_profile.symbol,IsLocalPaperModeActive(),g_paper_position,g_state,g_market,g_latency,g_ml_bridge);
         AppendGBPUSDDecisionEvent(now,"EXEC_SEND","SKIP","LIVE_SEND_DISABLED",g_market.spread_points,signal.score,risk_plan.lots,0,true,30);
        }
      else
        {
         ...
         AppendGBPUSDCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendGBPUSDDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendGBPUSDDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
   ...
   if(trans.deal > 0)
     {
      ...
      if(live_close_result.live_close_processed)
        {
         AppendGBPUSDDecisionEvent(now,"EXECUTION_TRUTH_CLOSE",(live_close_result.truth_close_written ? "OK" : "FAIL"),live_close_result.chain_reason,g_market.spread_points,0.0,deal_lots,0,false);
         AppendGBPUSDDecisionEvent(now,"LESSON_WRITE",(live_close_result.lesson_written ? "OK" : "FAIL"),"LIVE_DEAL_CLOSE",g_market.spread_points,0.0,deal_lots,0,false);
        }
