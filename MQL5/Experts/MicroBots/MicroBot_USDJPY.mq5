...
#property description "MicroBot USDJPY scaffold."
...
#include "..\\..\\Include\\Profiles\\Profile_USDJPY.mqh"
#include "..\\..\\Include\\Strategies\\Strategy_USDJPY.mqh"

input ulong InpMagic = 910103;
...
input string InpTradeComment = "MB_USDJPY";
input bool InpEnableStrategyTesterSandbox = true;
input string InpStrategyTesterSandboxTag = "USDJPY_AGENT";
...
MbTuningLocalPolicy g_usdjpy_local_tuning_policy;
MbTuningLocalPolicy g_usdjpy_effective_tuning_policy;
MbTuningFamilyPolicy g_usdjpy_family_tuning_policy;
MbTuningCoordinatorState g_usdjpy_coordinator_state;
...
bool ShouldRunUSDJPYTuningCycle(const datetime now)
  {
   const int tuning_service_interval_sec = 300;

   if(!g_usdjpy_local_tuning_policy.enabled)
      return false;

   if(g_usdjpy_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_usdjpy_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_usdjpy_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void AppendUSDJPYCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendUSDJPYAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   ...
   AppendUSDJPYDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }

double ApplyUSDJPYRiskMultiplierToLots(const MbMarketSnapshot &snapshot,const double base_lots,const double multiplier)
  {
...
int ResolveUSDJPaperHoldSeconds(const MbSignalDecision &signal)
  {
   ...
   else if(signal.setup_type == "SETUP_RANGE" && signal.market_regime == "RANGE" && signal.confidence_bucket != "LOW")
      hold_seconds = 360;
...
int OnInit()
  {
   ...
   LoadProfileUSDJPY(g_profile);
   if(!MbVerifyChartSymbol(g_profile.symbol))
      return(INIT_FAILED);
   g_profile.symbol = Symbol();
   ConfigureUSDJPYStrategyTesterSandbox();
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyUSDJPYInit(g_profile))
      return(INIT_FAILED);
   ...
   NormalizeUSDJPYMarketPermissions();
   ...
   MbTuningLocalPolicyReset(g_usdjpy_local_tuning_policy);
   MbLoadTuningLocalPolicy(g_profile.symbol,g_usdjpy_local_tuning_policy);
   MbTuningLocalPolicyReset(g_usdjpy_effective_tuning_policy);
   MbTuningFamilyPolicyReset(g_usdjpy_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_usdjpy_coordinator_state);
   MbBuildEffectiveTuningPolicy(g_profile.session_profile,g_usdjpy_local_tuning_policy,g_usdjpy_effective_tuning_policy,g_usdjpy_family_tuning_policy,g_usdjpy_coordinator_state);
   StrategyUSDJPYSetTuningPolicy(g_usdjpy_effective_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_usdjpy_effective_tuning_policy);
   ...
   PrintFormat(
      "MB_USDJPY_ONNX_OBSERVATION enabled=%s ready=%s symbol=%s",
...
void OnDeinit(const int reason)
  {
   ...
   MbTesterTelemetryFinalizeSingleRun(g_profile,g_state,g_market,g_usdjpy_effective_tuning_policy,g_latency);
   ...
   MbSaveTuningLocalPolicy(g_profile.symbol,g_usdjpy_local_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_usdjpy_effective_tuning_policy);
   StrategyUSDJPYDeinit();
...
void OnTimer()
  {
   ...
   if(MbShouldForceGlobalTeacherLearningTimerScan(g_profile.symbol,IsLocalPaperModeActive(),now))
     {
      AppendUSDJPYDecisionEvent(now,"DIAGNOSTIC","FORCE","TIMER_FALLBACK_SCAN",g_market.spread_points,0.0,0.0,0,true,60);
   ...
   MbFlushInformationalPolicy(g_profile,g_state,g_market,g_usdjpy_local_tuning_policy,"BOOTSTRAP",(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbFlushBrokerProfile(g_profile,g_state,g_market);
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_usdjpy_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbMlRuntimeBridgeFlushSnapshot(g_ml_bridge,g_profile,g_market,g_latency);
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   ...
   if(ShouldRunUSDJPYTuningCycle(now))
     {
      ...
      if(MbGetTuningFamilySymbols(g_profile.session_profile,family_symbols) > 0)
        {
         MbLoadTuningFamilyPolicy(g_profile.session_profile,g_usdjpy_family_tuning_policy);
         family_changed = MbRunTuningFamilyAgent(g_profile.session_profile,family_symbols,g_usdjpy_family_tuning_policy,family_reason);
        }

      if(MbGetAllTuningFamilies(families) > 0)
        {
         MbLoadTuningCoordinatorState(g_usdjpy_coordinator_state);
         coordinator_changed = MbRunTuningCoordinator(families,g_usdjpy_coordinator_state,coordinator_reason);
        }

      MbApplyGlobalTeacherLearningTuningRescue(g_profile.symbol,IsLocalPaperModeActive(),g_usdjpy_family_tuning_policy,g_usdjpy_coordinator_state);

      MbTuningDeckhandReport tuning_deckhand;
      MbTuningDeckhandReportReset(tuning_deckhand);
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_usdjpy_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_usdjpy_family_tuning_policy,g_usdjpy_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_usdjpy_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendUSDJPYDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_usdjpy_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_usdjpy_family_tuning_policy.freeze_new_changes)
         AppendUSDJPYDecisionEvent(now,"TUNING_FAMILY","SKIP",g_usdjpy_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendUSDJPYDecisionEvent(now,"TUNING_FLEET","ADJUST",g_usdjpy_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_usdjpy_coordinator_state.freeze_new_changes)
         AppendUSDJPYDecisionEvent(now,"TUNING_FLEET","SKIP",g_usdjpy_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendUSDJPYDecisionEvent(now,"TUNING","ADJUST",g_usdjpy_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendUSDJPYDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_usdjpy_local_tuning_policy.trusted_data)
         AppendUSDJPYDecisionEvent(now,"TUNING","SKIP",g_usdjpy_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
   MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
   MbSaveRuntimeState(g_state);
...
double OnTester()
  {
   return MbTesterTelemetryOnTester(g_profile,g_state,g_market,g_usdjpy_effective_tuning_policy,g_latency);
  }
...
void OnTick()
  {
      ...
      if(MbPaperMaybeClosePosition(g_market,g_paper_position,now,paper_pnl,paper_close_reason,closed_paper))
        {
         ...
         AppendUSDJPYDecisionEvent(now,"EXECUTION_TRUTH_CLOSE",(close_result.truth_close_written ? "OK" : "FAIL"),close_result.chain_reason,g_market.spread_points,0.0,0.0,0,false);
         AppendUSDJPYDecisionEvent(now,"LESSON_WRITE",(close_result.lesson_written ? "OK" : "FAIL"),close_result.chain_reason,g_market.spread_points,0.0,0.0,0,false);
         AppendUSDJPYDecisionEvent(now,"KNOWLEDGE_WRITE",(close_result.knowledge_bridge_enabled ? (close_result.knowledge_written ? "OK" : "FAIL") : "SKIP"),(close_result.knowledge_bridge_enabled ? close_result.chain_reason : "ML_BRIDGE_DISABLED"),g_market.spread_points,0.0,0.0,0,false);
         AppendUSDJPYDecisionEvent(now,"PAPER_CLOSE",(paper_pnl >= 0.0 ? "OK" : "LOSS"),paper_close_reason,g_market.spread_points,closed_paper.confidence_score,paper_pnl,0,false);
         MbSavePaperPosition(g_profile.symbol,g_paper_position);
   ...
   ManageUSDJPYOpenPosition(g_trade,g_state,g_profile,g_market);
      ...
      if(MbShouldBypassGlobalTeacherLearningRateGuard(g_profile.symbol,IsLocalPaperModeActive(),rate_guard.reason_code))
        {
         ...
         MbMicrobotHooksWriteSnapshot(g_hook_state,g_profile.symbol,IsLocalPaperModeActive(),g_paper_position,g_state,g_market,g_latency,g_ml_bridge);
         AppendUSDJPYDecisionEvent(now,"RATE_GUARD","BYPASS",("PAPER_IGNORE_" + rate_guard.reason_code),g_market.spread_points,0.0,0.0,0,true,60);
        }
      else
        {
         g_state.halt = true;
   ...
   if(MbShouldBypassMarketGuardInPaper(IsLocalPaperModeActive(),guard_reason))
     {
      AppendUSDJPYDecisionEvent(now,"MARKET","BYPASS",("PAPER_IGNORE_" + guard_reason),g_market.spread_points,0.0,0.0,0,true,60);
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendUSDJPYDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendUSDJPYDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   MbRefreshPaperTradeRights(g_state,IsLocalPaperModeActive());
   EvaluateUSDJPYStrategy(g_state,g_profile,g_market,signal);
   ...
   if(signal.setup_type != "NONE")
      AppendUSDJPYAuxDecisionEvent(now,signal,(signal.score >= 0.0 ? MB_SIGNAL_BUY : MB_SIGNAL_SELL));
   ...
   if(IsLocalPaperModeActive() && !signal.valid && MbShouldBypassGlobalTeacherLearningSoftReject(g_profile.symbol,IsLocalPaperModeActive(),signal.setup_type,signal.reason_code))
     {
      ...
      if(breakout_setup)
        {
         ...
         if(
            ...
            paper_gate_abs = MathMin(paper_gate_abs,0.70);
      ...
      if(MbShouldRelaxGlobalTeacherLearningCostGate(g_profile.symbol,IsLocalPaperModeActive()))
        {
         blocked_by_usdjpy_breakout_chaos_gate = false;
         blocked_by_usdjpy_range_chaos_gate = false;
        }

      if(!blocked_by_tuning_gate && !blocked_by_usdjpy_breakout_chaos_gate && !blocked_by_usdjpy_range_chaos_gate && (global_teacher_diag_active || MathAbs(signal.score) >= paper_gate_abs))
        {
         signal.valid = true;
   ...
   UsdJpyLocalRiskPlan risk_plan;
   BuildUSDJPYRiskPlan(g_state,g_market,risk_plan);
   ...
   if(
      ...
      MathAbs(signal.score) >= 0.52 &&
      (signal.market_regime == "CHAOS" || signal.market_regime == "RANGE") &&
      signal.execution_regime == "GOOD" &&
      signal.renko_quality_grade != "POOR" &&
      signal.candle_quality_grade != "UNKNOWN" &&
      g_market.spread_points <= 20.0
   )
     {
      risk_plan.allowed = true;
      risk_plan.reason_code = "PAPER_IGNORE_RISK_BLOCK_ASIA_RANGE";
      risk_plan.lots = MathMax(g_market.vol_min,g_market.vol_step);
     }
   if(signal.valid)
      risk_plan.lots = ApplyUSDJPYRiskMultiplierToLots(g_market,risk_plan.lots,signal.risk_multiplier);
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      AppendUSDJPYCandidateEvent(now,"SIZE_BLOCK",false,risk_plan.reason_code,signal,0.0);
      AppendUSDJPYDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
   ...
   if(signal.valid && (MbHasPosition(g_state.symbol,g_state.magic) || (IsLocalPaperModeActive() && MbPaperHasOpenPosition(g_paper_position))))
     {
      AppendUSDJPYDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,signal.score,0.0,0,true,60);
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendUSDJPYCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendUSDJPYDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
      ...
      AppendUSDJPYDecisionEvent(now,"EXEC_PRECHECK","READY","PRECHECK_OK",g_market.spread_points,signal.score,risk_plan.lots,0,false);
      ...
      if(IsLocalPaperModeActive())
        {
         ...
         bool paper_opened = MbLearningOutcomeHandlePaperOpen(
            ...
            g_market,
            now,
            ResolveUSDJPaperHoldSeconds(signal),
            InpEnableLiveEntries,
            paper_open_result
         );
         AppendUSDJPYDecisionEvent(now,"EXECUTION_TRUTH_OPEN",(paper_open_result.truth_open_written ? "OK" : "FAIL"),(paper_open_result.truth_open_written ? paper_open_result.candidate_id : "PAPER_OPEN_TRUTH_WRITE_FAIL"),g_market.spread_points,signal.score,risk_plan.lots,0,false);
         AppendUSDJPYDecisionEvent(now,"PAPER_POSITION_SAVE",(paper_open_result.paper_saved ? "OK" : "FAIL"),(paper_open_result.paper_saved ? (paper_opened ? paper_open_result.candidate_id : paper_open_result.reason_code) : "PAPER_POSITION_SAVE_FAIL"),g_market.spread_points,signal.score,risk_plan.lots,0,false);
         if(paper_opened)
           {
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
            AppendUSDJPYCandidateEvent(now,"PAPER_OPEN",true,"PAPER_POSITION_OPENED",signal,risk_plan.lots);
            AppendUSDJPYDecisionEvent(now,"PAPER_OPEN","OK","PAPER_POSITION_OPENED",g_market.spread_points,signal.score,risk_plan.lots,0,false);
           }
         else
           {
            AppendUSDJPYCandidateEvent(now,"PAPER_OPEN",false,paper_open_result.reason_code,signal,risk_plan.lots);
            AppendUSDJPYDecisionEvent(now,"PAPER_OPEN","SKIP",paper_open_result.reason_code,g_market.spread_points,signal.score,risk_plan.lots,0,false);
           }
         ...
         MbMicrobotHooksWriteSnapshot(g_hook_state,g_profile.symbol,IsLocalPaperModeActive(),g_paper_position,g_state,g_market,g_latency,g_ml_bridge);
         AppendUSDJPYDecisionEvent(now,"EXEC_SEND","SKIP","LIVE_SEND_DISABLED",g_market.spread_points,signal.score,risk_plan.lots,0,true,30);
        }
      else
        {
         ...
         AppendUSDJPYCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendUSDJPYDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendUSDJPYDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
   ...
   if(trans.deal > 0)
     {
      ...
      if(live_close_result.live_close_processed)
        {
         AppendUSDJPYDecisionEvent(now,"EXECUTION_TRUTH_CLOSE",(live_close_result.truth_close_written ? "OK" : "FAIL"),live_close_result.chain_reason,g_market.spread_points,0.0,deal_lots,0,false);
         AppendUSDJPYDecisionEvent(now,"LESSON_WRITE",(live_close_result.lesson_written ? "OK" : "FAIL"),"LIVE_DEAL_CLOSE",g_market.spread_points,0.0,deal_lots,0,false);
        }
