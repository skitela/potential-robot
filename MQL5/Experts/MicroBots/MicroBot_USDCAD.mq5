...
#property description "MicroBot USDCAD scaffold."
...
#include "..\\..\\Include\\Profiles\\Profile_USDCAD.mqh"
#include "..\\..\\Include\\Strategies\\Strategy_USDCAD.mqh"

input ulong InpMagic = 910105;
...
input string InpTradeComment = "MB_USDCAD";
input bool InpEnableStrategyTesterSandbox = true;
input string InpStrategyTesterSandboxTag = "USDCAD_AGENT";
...
MbTuningLocalPolicy g_usdcad_local_tuning_policy;
MbTuningLocalPolicy g_usdcad_effective_tuning_policy;
MbTuningFamilyPolicy g_usdcad_family_tuning_policy;
MbTuningCoordinatorState g_usdcad_coordinator_state;
...
bool ShouldRunUSDCADTimerDiagnosticScan(const datetime now)
  {
...
bool ShouldRunUSDCADTuningCycle(const datetime now)
  {
   const int tuning_service_interval_sec = 300;

   if(!g_usdcad_local_tuning_policy.enabled)
      return false;

   if(g_usdcad_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_usdcad_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_usdcad_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void ConfigureUSDCADStrategyTesterSandbox()
  {
...
void EnsureUSDCADDecisionHeartbeat(const datetime ts,const MbOnnxObservationResult &onnx_result)
  {
...
void AppendUSDCADCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendUSDCADAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   ...
   AppendUSDCADDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }

double ApplyUSDCADRiskMultiplierToLots(const MbMarketSnapshot &snapshot,const double base_lots,const double multiplier)
  {
...
int ResolveUSDCADPaperHoldSeconds(const MbSignalDecision &signal)
  {
...
int OnInit()
  {
   ...
   LoadProfileUSDCAD(g_profile);
   if(!MbVerifyChartSymbol(g_profile.symbol))
      return(INIT_FAILED);
   g_profile.symbol = Symbol();
   ConfigureUSDCADStrategyTesterSandbox();
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyUSDCADInit(g_profile))
      return(INIT_FAILED);
   ...
   NormalizeUSDCADMarketPermissions();
   ...
   MbTuningLocalPolicyReset(g_usdcad_local_tuning_policy);
   MbLoadTuningLocalPolicy(g_profile.symbol,g_usdcad_local_tuning_policy);
   MbTuningLocalPolicyReset(g_usdcad_effective_tuning_policy);
   MbTuningFamilyPolicyReset(g_usdcad_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_usdcad_coordinator_state);
   MbBuildEffectiveTuningPolicy(g_profile.session_profile,g_usdcad_local_tuning_policy,g_usdcad_effective_tuning_policy,g_usdcad_family_tuning_policy,g_usdcad_coordinator_state);
   StrategyUSDCADSetTuningPolicy(g_usdcad_effective_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_usdcad_effective_tuning_policy);
   ...
   PrintFormat(
      "MB_USDCAD_ONNX_OBSERVATION enabled=%s ready=%s symbol=%s",
...
void OnDeinit(const int reason)
  {
   ...
   MbTesterTelemetryFinalizeSingleRun(g_profile,g_state,g_market,g_usdcad_effective_tuning_policy,g_latency);
   ...
   MbSaveTuningLocalPolicy(g_profile.symbol,g_usdcad_local_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_usdcad_effective_tuning_policy);
   StrategyUSDCADDeinit();
...
void OnTimer()
  {
   ...
   if(ShouldRunUSDCADTimerDiagnosticScan(now))
     {
      g_last_timer_diagnostic_scan_ts = now;
      AppendUSDCADDecisionEvent(now,"DIAGNOSTIC","FORCE","TIMER_FALLBACK_SCAN",g_market.spread_points,0.0,0.0,0,true,60);
   ...
   EnsureUSDCADDecisionHeartbeat(now,timer_onnx_result);
   ...
   MbFlushInformationalPolicy(g_profile,g_state,g_market,g_usdcad_local_tuning_policy,"BOOTSTRAP",(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbFlushBrokerProfile(g_profile,g_state,g_market);
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_usdcad_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbMlRuntimeBridgeFlushSnapshot(g_ml_bridge,g_profile,g_market,g_latency);
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   ...
   if(ShouldRunUSDCADTuningCycle(now))
     {
      ...
      if(MbGetAllTuningFamilies(families) > 0)
        {
         MbLoadTuningCoordinatorState(g_usdcad_coordinator_state);
         coordinator_changed = MbRunTuningCoordinator(families,g_usdcad_coordinator_state,coordinator_reason);
        }

      MbTuningDeckhandReport tuning_deckhand;
      MbTuningDeckhandReportReset(tuning_deckhand);
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_usdcad_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_usdcad_family_tuning_policy,g_usdcad_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_usdcad_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendUSDCADDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_usdcad_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_usdcad_family_tuning_policy.freeze_new_changes)
         AppendUSDCADDecisionEvent(now,"TUNING_FAMILY","SKIP",g_usdcad_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendUSDCADDecisionEvent(now,"TUNING_FLEET","ADJUST",g_usdcad_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_usdcad_coordinator_state.freeze_new_changes)
         AppendUSDCADDecisionEvent(now,"TUNING_FLEET","SKIP",g_usdcad_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendUSDCADDecisionEvent(now,"TUNING","ADJUST",g_usdcad_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendUSDCADDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_usdcad_local_tuning_policy.trusted_data)
         AppendUSDCADDecisionEvent(now,"TUNING","SKIP",g_usdcad_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
...
double OnTester()
  {
   return MbTesterTelemetryOnTester(g_profile,g_state,g_market,g_usdcad_effective_tuning_policy,g_latency);
  }
...
void OnTick()
  {
      ...
      if(MbPaperMaybeClosePosition(g_market,g_paper_position,now,paper_pnl,paper_close_reason,closed_paper))
        {
         ...
         AppendUSDCADDecisionEvent(now,"PAPER_CLOSE",(paper_pnl >= 0.0 ? "OK" : "LOSS"),paper_close_reason,g_market.spread_points,0.0,paper_pnl,0,false);
         MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
        }
     }

   ManageUSDCADOpenPosition(g_trade,g_state,g_profile,g_market);
      ...
      if(MbShouldBypassFirstWaveTruthDiagnosticRateGuard(g_profile.symbol,IsLocalPaperModeActive(),rate_guard.reason_code))
        {
         ...
         AppendUSDCADDecisionEvent(now,"RATE_GUARD","BYPASS",("PAPER_IGNORE_" + rate_guard.reason_code),g_market.spread_points,0.0,0.0,0,true,60);
        }
      else
        {
         g_state.halt = true;
   ...
   if(already_has_position)
     {
      AppendUSDCADDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,0.0,0.0,0,true,300);
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendUSDCADDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
      return;
     }
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendUSDCADDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   if(signal.setup_type != "NONE")
      AppendUSDCADAuxDecisionEvent(now,signal,(signal.score >= 0.0 ? MB_SIGNAL_BUY : MB_SIGNAL_SELL));
   ...
   if(IsLocalPaperModeActive() && soft_diag_reject)
     {
      ...
      if(signal.setup_type == "SETUP_BREAKOUT" && g_usdcad_effective_tuning_policy.require_non_poor_candle_for_breakout && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_usdcad_effective_tuning_policy.require_non_poor_renko_for_breakout && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_usdcad_effective_tuning_policy.require_non_poor_candle_for_range && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_usdcad_effective_tuning_policy.require_non_poor_renko_for_range && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_usdcad_effective_tuning_policy.range_confidence_floor > 0.0 && signal.confidence_score < g_usdcad_effective_tuning_policy.range_confidence_floor)
         blocked_by_tuning_gate = true;
      if(
         ...
         blocked_by_usdcad_breakout_chaos_cost_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT")
        {
         paper_gate_abs = 0.62;
      ...
      else if(blocked_by_usdcad_breakout_chaos_cost_gate)
         signal.reason_code = "USDCAD_BREAKOUT_CHAOS_BAD_SPREAD_BLOCK";
     }
   ...
   AppendUSDCADCandidateEvent(now,"EVALUATED",signal.valid,signal.reason_code,signal,0.0);
   if(!signal.valid)
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);

   UsdCadLocalRiskPlan risk_plan;
   BuildUSDCADRiskPlan(g_state,g_market,risk_plan);
   ...
   if(signal.valid)
      risk_plan.lots = ApplyUSDCADRiskMultiplierToLots(g_market,risk_plan.lots,signal.risk_multiplier);
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      AppendUSDCADCandidateEvent(now,"SIZE_BLOCK",false,risk_plan.reason_code,signal,0.0);
      AppendUSDCADDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
      return;
     }
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendUSDCADCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendUSDCADDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         return;
        }

      AppendUSDCADDecisionEvent(now,"EXEC_PRECHECK","READY","PRECHECK_OK",g_market.spread_points,signal.score,risk_plan.lots,0,false);
         ...
         if(IsLocalPaperModeActive())
           {
            ...
            bool paper_opened = MbPaperOpenPosition(
               ...
               g_market.spread_points,
               now,
               ResolveUSDCADPaperHoldSeconds(signal),
               signal.reason_code,
               signal.setup_type,
            ...
            if(paper_opened)
              {
               ...
               AppendUSDCADCandidateEvent(now,"PAPER_OPEN",true,"PAPER_POSITION_OPENED",signal,risk_plan.lots);
               AppendUSDCADDecisionEvent(now,"PAPER_OPEN","OK","PAPER_POSITION_OPENED",g_market.spread_points,signal.score,risk_plan.lots,0,false);
              }
            else
              {
               ...
               AppendUSDCADCandidateEvent(now,"PAPER_OPEN",false,paper_open_reason,signal,risk_plan.lots);
               AppendUSDCADDecisionEvent(now,"PAPER_OPEN","SKIP",paper_open_reason,g_market.spread_points,signal.score,risk_plan.lots,0,false);
              }
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         }
         AppendUSDCADDecisionEvent(now,"EXEC_SEND","SKIP","LIVE_SEND_DISABLED",g_market.spread_points,signal.score,risk_plan.lots,0,true,30);
        }
      else
        {
         ...
         if(MbMlRuntimeBridgeBlocksLiveExecution(g_ml_bridge,live_scope_reason))
           {
            AppendUSDCADDecisionEvent(now,"EXEC_SCOPE","BLOCK",live_scope_reason,g_market.spread_points,signal.score,risk_plan.lots,0,false);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
            return;
           }
         MbMarkOrderSend(g_state);
         ...
         AppendUSDCADCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendUSDCADDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendUSDCADDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
  }
