...
#include "..\\..\\Include\\Profiles\\Profile_AUDUSD.mqh"
#include "..\\..\\Include\\Strategies\\Strategy_AUDUSD.mqh"

input ulong InpMagic = 910104;
...
input string InpTradeComment = "MB_AUDUSD";
input bool InpEnableStrategyTesterSandbox = true;
input string InpStrategyTesterSandboxTag = "AUDUSD_AGENT";
...
MbTuningLocalPolicy g_audusd_local_tuning_policy;
MbTuningLocalPolicy g_audusd_effective_tuning_policy;
MbTuningFamilyPolicy g_audusd_family_tuning_policy;
MbTuningCoordinatorState g_audusd_coordinator_state;
...
bool ShouldRunAUDUSDTuningCycle(const datetime now)
  {
   ...
   if(!g_audusd_local_tuning_policy.enabled)
      return false;

   if(g_audusd_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_audusd_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_audusd_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void ConfigureAUDUSDStrategyTesterSandbox()
  {
...
void AppendAUDUSDCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendAUDUSDAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   ...
   AppendAUDUSDDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }

double ApplyAUDUSDRiskMultiplierToLots(const MbMarketSnapshot &snapshot,const double base_lots,const double multiplier)
  {
...
int ResolveAUDUSDPaperHoldSeconds(const MbSignalDecision &signal)
  {
      ...
      if(StringFind(signal.reason_code,"PAPER_SCORE_GATE",0) == 0 || signal.confidence_bucket == "LOW")
         hold_seconds = 180;
              ...
              (signal.renko_quality_grade == "POOR" ||
               (signal.renko_quality_grade == "UNKNOWN" && signal.candle_quality_grade != "GOOD")))
         hold_seconds = 150;
...
int OnInit()
  {
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyAUDUSDInit(g_profile))
      return(INIT_FAILED);
   ...
   MbTuningFamilyPolicyReset(g_audusd_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_audusd_coordinator_state);
   MbBuildEffectiveTuningPolicy(g_profile.session_profile,g_audusd_local_tuning_policy,g_audusd_effective_tuning_policy,g_audusd_family_tuning_policy,g_audusd_coordinator_state);
   StrategyAUDUSDSetTuningPolicy(g_audusd_effective_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_audusd_effective_tuning_policy);
   ...
   PrintFormat(
      "MB_AUDUSD_ONNX_OBSERVATION enabled=%s ready=%s symbol=%s",
...
void OnDeinit(const int reason)
  {
   ...
   MbTesterTelemetryFinalizeSingleRun(g_profile,g_state,g_market,g_audusd_effective_tuning_policy,g_latency);
   ...
   MbSaveTuningLocalPolicy(g_profile.symbol,g_audusd_local_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_audusd_effective_tuning_policy);
   StrategyAUDUSDDeinit();
...
void OnTimer()
  {
   ...
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_audusd_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbMlRuntimeBridgeFlushSnapshot(g_ml_bridge,g_profile,g_market,g_latency);
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   ...
   if(ShouldRunAUDUSDTuningCycle(now))
     {
      ...
      if(MbGetTuningFamilySymbolsForRuntime(g_profile.symbol,g_profile.session_profile,IsLocalPaperModeActive(),family_symbols) > 0)
        {
         MbLoadTuningFamilyPolicy(g_profile.session_profile,g_audusd_family_tuning_policy);
         family_changed = MbRunTuningFamilyAgent(g_profile.session_profile,family_symbols,g_audusd_family_tuning_policy,family_reason);
        }

      if(MbGetAllTuningFamilies(families) > 0)
        {
         MbLoadTuningCoordinatorState(g_audusd_coordinator_state);
         coordinator_changed = MbRunTuningCoordinator(families,g_audusd_coordinator_state,coordinator_reason);
        }

      MbTuningDeckhandReport tuning_deckhand;
      MbTuningDeckhandReportReset(tuning_deckhand);
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_audusd_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_audusd_family_tuning_policy,g_audusd_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_audusd_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendAUDUSDDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_audusd_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_audusd_family_tuning_policy.freeze_new_changes)
         AppendAUDUSDDecisionEvent(now,"TUNING_FAMILY","SKIP",g_audusd_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendAUDUSDDecisionEvent(now,"TUNING_FLEET","ADJUST",g_audusd_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_audusd_coordinator_state.freeze_new_changes)
         AppendAUDUSDDecisionEvent(now,"TUNING_FLEET","SKIP",g_audusd_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendAUDUSDDecisionEvent(now,"TUNING","ADJUST",g_audusd_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendAUDUSDDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_audusd_local_tuning_policy.trusted_data)
         AppendAUDUSDDecisionEvent(now,"TUNING","SKIP",g_audusd_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
   MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
   MbSaveRuntimeState(g_state);
...
void OnTick()
  {
      ...
      if(MbPaperMaybeClosePosition(g_market,g_paper_position,now,paper_pnl,paper_close_reason,closed_paper))
        {
         ...
         AppendAUDUSDDecisionEvent(now,"PAPER_CLOSE",(paper_pnl >= 0.0 ? "OK" : "LOSS"),paper_close_reason,g_market.spread_points,0.0,paper_pnl,0,false);
         MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
        }
     }
   ...
   if(already_has_position)
     {
      AppendAUDUSDDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,0.0,0.0,0,true,300);
   ...
   if(MbShouldBypassMarketGuardInPaperForSymbol(g_profile.symbol,IsLocalPaperModeActive(),guard_reason))
     {
      ...
      AppendAUDUSDDecisionEvent(now,"MARKET","BYPASS",("PAPER_IGNORE_" + guard_reason),g_market.spread_points,0.0,0.0,0,true,market_bypass_throttle);
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendAUDUSDDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
      return;
     }
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendAUDUSDDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   if(IsLocalPaperModeActive() && soft_diag_reject)
     {
      ...
      if(signal.setup_type == "SETUP_BREAKOUT" && g_audusd_effective_tuning_policy.require_non_poor_candle_for_breakout && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_audusd_effective_tuning_policy.require_non_poor_renko_for_breakout && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_audusd_effective_tuning_policy.require_non_poor_candle_for_range && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_audusd_effective_tuning_policy.require_non_poor_renko_for_range && poor_renko)
         blocked_by_tuning_gate = true;
      ...
      if(signal.setup_type == "SETUP_BREAKOUT")
        ...
        {
         paper_gate_abs = 0.19;
         if((signal.market_regime == "CHAOS" || signal.market_regime == "TREND") &&
            poor_candle &&
            signal.renko_quality_grade != "GOOD")
            blocked_by_audusd_dirty_range_gate = true;
        }
   ...
   AudUsdLocalRiskPlan risk_plan;
   BuildAUDUSDRiskPlan(g_state,g_market,risk_plan);
   ...
   if(
      ...
      MathAbs(signal.score) >= 0.98 &&
      signal.execution_regime == "GOOD" &&
      (signal.market_regime == "CHAOS" || signal.market_regime == "BREAKOUT") &&
      signal.renko_quality_grade == "GOOD" &&
   ...
   )
     {
      ...
      risk_plan.reason_code = "PAPER_IGNORE_RISK_BLOCK_ASIA_BREAKOUT_SELL";
      risk_plan.lots = MathMax(g_market.vol_min,g_market.vol_step);
     }
   if(signal.valid)
      risk_plan.lots = ApplyAUDUSDRiskMultiplierToLots(g_market,risk_plan.lots,signal.risk_multiplier);
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      AppendAUDUSDCandidateEvent(now,"SIZE_BLOCK",false,risk_plan.reason_code,signal,0.0);
      AppendAUDUSDDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
      return;
     }
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendAUDUSDCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendAUDUSDDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         return;
        }

      AppendAUDUSDDecisionEvent(now,"EXEC_PRECHECK","READY","PRECHECK_OK",g_market.spread_points,signal.score,risk_plan.lots,0,false);
         ...
         if(IsLocalPaperModeActive())
           {
            ...
            bool paper_opened = MbPaperOpenPosition(
               ...
               g_market.spread_points,
               now,
               ResolveAUDUSDPaperHoldSeconds(signal),
               signal.reason_code,
               signal.setup_type,
            ...
            if(paper_opened)
              {
               ...
               AppendAUDUSDDecisionEvent(now,"PAPER_OPEN","OK","PAPER_POSITION_OPENED",g_market.spread_points,signal.score,risk_plan.lots,0,false);
              }
            else
              {
               ...
               AppendAUDUSDCandidateEvent(now,"PAPER_OPEN",false,paper_open_reason,signal,risk_plan.lots);
               AppendAUDUSDDecisionEvent(now,"PAPER_OPEN","SKIP",paper_open_reason,g_market.spread_points,signal.score,risk_plan.lots,0,false);
              }
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         }
         AppendAUDUSDDecisionEvent(now,"EXEC_SEND","SKIP","LIVE_SEND_DISABLED",g_market.spread_points,signal.score,risk_plan.lots,0,true,30);
        }
      else
        {
         ...
         if(MbMlRuntimeBridgeBlocksLiveExecution(g_ml_bridge,live_scope_reason))
           {
            AppendAUDUSDDecisionEvent(now,"EXEC_SCOPE","BLOCK",live_scope_reason,g_market.spread_points,signal.score,risk_plan.lots,0,false);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
            return;
           }
         MbMarkOrderSend(g_state);
         ...
         AppendAUDUSDCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendAUDUSDDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendAUDUSDDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
  }
