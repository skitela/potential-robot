...
#property description "MicroBot NZDUSD scaffold."
...
MbTuningLocalPolicy g_nzdusd_local_tuning_policy;
MbTuningLocalPolicy g_nzdusd_effective_tuning_policy;
MbTuningFamilyPolicy g_nzdusd_family_tuning_policy;
MbTuningCoordinatorState g_nzdusd_coordinator_state;
...
bool ShouldRunNZDUSDTuningCycle(const datetime now)
  {
   ...
   if(!g_nzdusd_local_tuning_policy.enabled)
      return false;

   if(g_nzdusd_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_nzdusd_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_nzdusd_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void ConfigureNZDUSDStrategyTesterSandbox()
  {
...
void NormalizeNZDUSDMarketPermissions()
  {
...
void AppendNZDUSDDecisionEvent(
   const datetime ts,
   const string phase,
...
void AppendNZDUSDCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendNZDUSDAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   if(signal.setup_type == "NONE")
      return;
   ...
   AppendNZDUSDDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }

double ApplyNZDUSDRiskMultiplierToLots(const MbMarketSnapshot &snapshot,const double base_lots,const double multiplier)
  {
...
int ResolveNZDUSDPaperHoldSeconds(const MbSignalDecision &signal)
  {
...
int OnInit()
  {
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyNZDUSDInit(g_profile))
      return(INIT_FAILED);
   ...
   NormalizeNZDUSDMarketPermissions();
   ...
   MbTuningLocalPolicyReset(g_nzdusd_local_tuning_policy);
   MbLoadTuningLocalPolicy(g_profile.symbol,g_nzdusd_local_tuning_policy);
   MbTuningLocalPolicyReset(g_nzdusd_effective_tuning_policy);
   MbTuningFamilyPolicyReset(g_nzdusd_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_nzdusd_coordinator_state);
...
void OnTimer()
  {
   ...
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_nzdusd_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   MbIncidentJournalFlush();
   ...
   if(ShouldRunNZDUSDTuningCycle(now))
     {
      ...
      if(MbGetTuningFamilySymbols(g_profile.session_profile,family_symbols) > 0)
        {
         MbLoadTuningFamilyPolicy(g_profile.session_profile,g_nzdusd_family_tuning_policy);
         family_changed = MbRunTuningFamilyAgent(g_profile.session_profile,family_symbols,g_nzdusd_family_tuning_policy,family_reason);
        }

      if(MbGetAllTuningFamilies(families) > 0)
        {
         MbLoadTuningCoordinatorState(g_nzdusd_coordinator_state);
         coordinator_changed = MbRunTuningCoordinator(families,g_nzdusd_coordinator_state,coordinator_reason);
        }

      MbTuningDeckhandReport tuning_deckhand;
      MbTuningDeckhandReportReset(tuning_deckhand);
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_nzdusd_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_nzdusd_family_tuning_policy,g_nzdusd_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_nzdusd_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendNZDUSDDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_nzdusd_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_nzdusd_family_tuning_policy.freeze_new_changes)
         AppendNZDUSDDecisionEvent(now,"TUNING_FAMILY","SKIP",g_nzdusd_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendNZDUSDDecisionEvent(now,"TUNING_FLEET","ADJUST",g_nzdusd_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_nzdusd_coordinator_state.freeze_new_changes)
         AppendNZDUSDDecisionEvent(now,"TUNING_FLEET","SKIP",g_nzdusd_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendNZDUSDDecisionEvent(now,"TUNING","ADJUST",g_nzdusd_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendNZDUSDDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_nzdusd_local_tuning_policy.trusted_data)
         AppendNZDUSDDecisionEvent(now,"TUNING","SKIP",g_nzdusd_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
   MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
   MbSaveRuntimeState(g_state);
...
void OnTick()
  {
   ...
   ManageNZDUSDOpenPosition(g_trade,g_state,g_profile,g_market);
   ...
   if(already_has_position)
     {
      AppendNZDUSDDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,0.0,0.0,0,true,300);
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendNZDUSDDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
      return;
     }
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendNZDUSDDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   EvaluateNZDUSDStrategy(g_state,g_profile,g_market,signal);
   if(signal.setup_type != "NONE")
     {
   ...
   if(signal.setup_type != "NONE")
      AppendNZDUSDAuxDecisionEvent(now,signal,(signal.score >= 0.0 ? MB_SIGNAL_BUY : MB_SIGNAL_SELL));
   ...
   if(IsLocalPaperModeActive() && !signal.valid && signal.reason_code == "SCORE_BELOW_TRIGGER")
     {
      ...
      bool blocked_by_nzdusd_breakout_dirty_gate = false;
      if(signal.setup_type == "SETUP_TREND" && g_nzdusd_effective_tuning_policy.require_non_poor_candle_for_trend && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_nzdusd_effective_tuning_policy.require_non_poor_candle_for_breakout && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_nzdusd_effective_tuning_policy.require_non_poor_renko_for_breakout && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_nzdusd_effective_tuning_policy.require_non_poor_candle_for_range && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_nzdusd_effective_tuning_policy.require_non_poor_renko_for_range && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_nzdusd_effective_tuning_policy.range_confidence_floor > 0.0 && signal.confidence_score < g_nzdusd_effective_tuning_policy.range_confidence_floor)
         blocked_by_tuning_gate = true;
      if(
         ...
         blocked_by_nzdusd_breakout_dirty_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT")
        {
         paper_gate_abs = 0.62;
         ...
         else if(poor_candle && (signal.market_regime == "CHAOS" || signal.market_regime == "BREAKOUT"))
   ...
   NzdUsdLocalRiskPlan risk_plan;
   BuildNZDUSDRiskPlan(g_state,g_market,risk_plan);
   ...
   if(signal.valid)
      risk_plan.lots = ApplyNZDUSDRiskMultiplierToLots(g_market,risk_plan.lots,signal.risk_multiplier);
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      AppendNZDUSDCandidateEvent(now,"SIZE_BLOCK",false,risk_plan.reason_code,signal,0.0);
      AppendNZDUSDDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
      return;
     }
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendNZDUSDCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendNZDUSDDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         return;
        }

      AppendNZDUSDDecisionEvent(now,"EXEC_PRECHECK","READY","PRECHECK_OK",g_market.spread_points,signal.score,risk_plan.lots,0,false);
         ...
         if(IsLocalPaperModeActive())
           {
            ...
            MbPaperOpenPosition(
               ...
               g_market.spread_points,
               now,
               ResolveNZDUSDPaperHoldSeconds(signal),
               signal.reason_code,
               signal.setup_type,
            ...
            MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
            AppendNZDUSDCandidateEvent(now,"PAPER_OPEN",true,"PAPER_POSITION_OPENED",signal,risk_plan.lots);
            AppendNZDUSDDecisionEvent(now,"PAPER_OPEN","OK","PAPER_POSITION_OPENED",g_market.spread_points,signal.score,risk_plan.lots,0,false);
         }
         AppendNZDUSDDecisionEvent(now,"EXEC_SEND","SKIP","LIVE_SEND_DISABLED",g_market.spread_points,signal.score,risk_plan.lots,0,true,30);
        }
      else
        {
         MbMarkOrderSend(g_state);
         ...
         AppendNZDUSDCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendNZDUSDDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendNZDUSDDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
  }
