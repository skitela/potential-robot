...
#property description "MicroBot PLATIN scaffold."
...
#include "..\\..\\Include\\Profiles\\Profile_PLATIN.mqh"
#include "..\\..\\Include\\Strategies\\Strategy_PLATIN.mqh"

input ulong InpMagic = 920203;
input uint InpTimerSec = 5;
input bool InpEnableLiveEntries = false;
input bool InpPaperCollectMode = true;
input string InpTradeComment = "MB_PLATIN";
input bool InpEnableStrategyTesterSandbox = true;
input string InpStrategyTesterSandboxTag = "PLATIN_AGENT";
...
MbTuningLocalPolicy g_PLATIN_local_tuning_policy;
MbTuningLocalPolicy g_PLATIN_effective_tuning_policy;
MbTuningFamilyPolicy g_PLATIN_family_tuning_policy;
...
bool ShouldRunPLATINTuningCycle(const datetime now)
  {
   const int tuning_service_interval_sec = 300;

   if(!g_PLATIN_local_tuning_policy.enabled)
      return false;

   if(g_PLATIN_local_tuning_policy.last_eval_at <= 0)
      return true;

   if(StringLen(g_tuning_deckhand_log_path) > 0 && !FileIsExist(g_tuning_deckhand_log_path,FILE_COMMON))
      return true;

   if(g_state.learning_sample_count != g_PLATIN_local_tuning_policy.last_learning_sample_count)
      return true;

   return ((now - g_PLATIN_local_tuning_policy.last_eval_at) >= tuning_service_interval_sec);
  }
...
void ConfigurePLATINStrategyTesterSandbox()
  {
...
void AppendPLATINCandidateEvent(
   const datetime ts,
   const string stage,
   const bool accepted,
   const string reason,
...
void AppendPLATINAuxDecisionEvent(const datetime ts,const MbSignalDecision &signal,const MbSignalSide intended_side)
  {
   if(signal.setup_type == "NONE")
      return;
   ...
   AppendPLATINDecisionEvent(ts,"AUX",verdict,reason,g_market.spread_points,signal.score,0.0,0,false);
  }
...
int ResolvePLATINPaperHoldSeconds(const MbSignalDecision &signal)
  {
   ...
   else if(signal.setup_type == "SETUP_RANGE" && signal.market_regime == "CHAOS")
     {
...
int OnInit()
  {
   ...
   LoadProfilePLATIN(g_profile);
   if(!MbVerifyChartSymbol(g_profile.symbol))
      return(INIT_FAILED);
   g_profile.symbol = Symbol();
   ConfigurePLATINStrategyTesterSandbox();
   ...
   if(!MbStorageInit(g_profile.symbol))
      return(INIT_FAILED);
   if(!StrategyPLATINInit(g_profile))
      return(INIT_FAILED);
   ...
   NormalizePLATINMarketPermissions();
   ...
   MbTuningLocalPolicyReset(g_PLATIN_local_tuning_policy);
   MbLoadTuningLocalPolicy(g_profile.symbol,g_PLATIN_local_tuning_policy);
   MbTuningLocalPolicyReset(g_PLATIN_effective_tuning_policy);
   MbTuningFamilyPolicyReset(g_PLATIN_family_tuning_policy);
   MbTuningCoordinatorStateReset(g_tuning_coordinator_state);
   MbBuildEffectiveTuningPolicy(g_profile.session_profile,g_PLATIN_local_tuning_policy,g_PLATIN_effective_tuning_policy,g_PLATIN_family_tuning_policy,g_tuning_coordinator_state);
   StrategyPLATINSetTuningPolicy(g_PLATIN_effective_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_PLATIN_effective_tuning_policy);
...
void OnDeinit(const int reason)
  {
   ...
   MbTesterTelemetryFinalizeSingleRun(g_profile,g_state,g_market,g_PLATIN_effective_tuning_policy,g_latency);
   ...
   MbSaveTuningLocalPolicy(g_profile.symbol,g_PLATIN_local_tuning_policy);
   MbSaveEffectiveTuningLocalPolicy(g_profile.symbol,g_PLATIN_effective_tuning_policy);
   StrategyPLATINDeinit();
...
void OnTimer()
  {
   ...
   MbFlushInformationalPolicy(g_profile,g_state,g_market,g_PLATIN_local_tuning_policy,"BOOTSTRAP",(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbFlushBrokerProfile(g_profile,g_state,g_market);
   MbFlushExecutionSummary(g_profile,g_state,g_market,g_PLATIN_local_tuning_policy,g_latency,(g_kill_switch.halt ? g_kill_switch.reason_code : g_runtime_control.reason_code));
   MbDecisionJournalFlush();
   MbCandidateSignalJournalFlush();
   MbExecutionTelemetryFlush();
   MbIncidentJournalFlush();
   ...
   if(ShouldRunPLATINTuningCycle(now))
     {
      ...
      if(MbGetTuningFamilySymbols(g_profile.session_profile,family_symbols) > 0)
        {
         MbLoadTuningFamilyPolicy(g_profile.session_profile,g_PLATIN_family_tuning_policy);
         family_changed = MbRunTuningFamilyAgent(g_profile.session_profile,family_symbols,g_PLATIN_family_tuning_policy,family_reason);
        }
      ...
      MbRunTuningDeckhand(g_profile.symbol,g_state,g_market,g_tuning_deckhand_log_path,g_PLATIN_local_tuning_policy,tuning_deckhand);

      string tuning_reason = "";
      string hierarchy_block_reason = "";
      bool hierarchy_blocks = MbTuningHierarchyBlocksLocalChanges(g_PLATIN_family_tuning_policy,g_tuning_coordinator_state,hierarchy_block_reason);
      if(IsLocalPaperModeActive())
         hierarchy_blocks = false;
      bool tuning_changed = false;
      if(IsLocalPaperModeActive())
         tuning_changed = MbRunLocalTuningAgent(g_profile.symbol,g_state,g_tuning_action_log_path,g_PLATIN_local_tuning_policy,tuning_deckhand,tuning_reason);
      ...
      if(family_changed)
         AppendPLATINDecisionEvent(now,"TUNING_FAMILY","ADJUST",g_PLATIN_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_PLATIN_family_tuning_policy.freeze_new_changes)
         AppendPLATINDecisionEvent(now,"TUNING_FAMILY","SKIP",g_PLATIN_family_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(coordinator_changed)
         AppendPLATINDecisionEvent(now,"TUNING_FLEET","ADJUST",g_tuning_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);
      else if(g_tuning_coordinator_state.freeze_new_changes)
         AppendPLATINDecisionEvent(now,"TUNING_FLEET","SKIP",g_tuning_coordinator_state.last_action_code,g_market.spread_points,0.0,0.0,0,true,600);

      if(tuning_changed)
         AppendPLATINDecisionEvent(now,"TUNING","ADJUST",g_PLATIN_local_tuning_policy.last_action_code,g_market.spread_points,0.0,0.0,0,true,300);
      else if(hierarchy_blocks)
         AppendPLATINDecisionEvent(now,"TUNING","SKIP",tuning_reason,g_market.spread_points,0.0,0.0,0,true,300);
      else if(!g_PLATIN_local_tuning_policy.trusted_data)
         AppendPLATINDecisionEvent(now,"TUNING","SKIP",g_PLATIN_local_tuning_policy.trust_reason,g_market.spread_points,0.0,0.0,0,true,300);
     }
...
void OnTick()
  {
      ...
      if(MbPaperMaybeClosePosition(g_market,g_paper_position,now,paper_pnl,paper_close_reason,closed_paper))
        {
         ...
         AppendPLATINDecisionEvent(now,"PAPER_CLOSE",(paper_pnl >= 0.0 ? "OK" : "LOSS"),paper_close_reason,g_market.spread_points,0.0,paper_pnl,0,false);
         MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
        }
     }
   ManagePLATINOpenPosition(g_trade,g_state,g_profile,g_market);
   ...
   if(already_has_position)
     {
      AppendPLATINDecisionEvent(now,"POSITION","SKIP","POSITION_ALREADY_OPEN",g_market.spread_points,0.0,0.0,0,true,300);
   ...
   if(MbShouldBypassMarketGuardInPaper(IsLocalPaperModeActive(),guard_reason))
     {
      ...
      AppendPLATINDecisionEvent(now,"MARKET","BYPASS",("PAPER_IGNORE_" + guard_reason),g_market.spread_points,0.0,0.0,0,true,market_bypass_throttle);
   ...
   if(market_guard != MB_GUARD_OK)
     {
      ...
      AppendPLATINDecisionEvent(now,"MARKET","SKIP",guard_reason,g_market.spread_points,0.0,0.0,0,true,60);
      return;
     }
   ...
   if(exec_quality_guard != MB_GUARD_OK)
     {
      ...
      AppendPLATINDecisionEvent(now,"EXEC_QUALITY",(exec_quality_guard == MB_GUARD_BLOCK ? "SKIP" : "CAUTION"),guard_reason,g_market.spread_points,0.0,0.0,0,true,30);
   ...
   MbRefreshPaperTradeRights(g_state,IsLocalPaperModeActive());
   EvaluatePLATINStrategy(g_state,g_profile,g_market,signal);
   if(signal.setup_type != "NONE")
     {
   ...
   if(signal.setup_type != "NONE")
      AppendPLATINAuxDecisionEvent(now,signal,(signal.score >= 0.0 ? MB_SIGNAL_BUY : MB_SIGNAL_SELL));
   if(IsLocalPaperModeActive() && !signal.valid && signal.reason_code == "SCORE_BELOW_TRIGGER")
     {
      ...
      if(signal.setup_type == "SETUP_TREND" && g_PLATIN_effective_tuning_policy.require_non_poor_candle_for_trend && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_PLATIN_effective_tuning_policy.require_non_poor_candle_for_breakout && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && g_PLATIN_effective_tuning_policy.require_non_poor_renko_for_breakout && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_PLATIN_effective_tuning_policy.require_non_poor_candle_for_range && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_PLATIN_effective_tuning_policy.require_non_poor_renko_for_range && poor_renko)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && g_PLATIN_effective_tuning_policy.range_confidence_floor > 0.0 && signal.confidence_score < g_PLATIN_effective_tuning_policy.range_confidence_floor)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && poor_candle && signal.confidence_score < 0.85)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT" && poor_candle && signal.renko_quality_grade != "GOOD")
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_RANGE" && signal.market_regime == "CHAOS" && signal.confidence_bucket == "LOW" && poor_candle)
         blocked_by_tuning_gate = true;
      if(signal.setup_type == "SETUP_BREAKOUT")
        {
         paper_gate_abs = 0.60;
         ...
         if(poor_candle && poor_renko)
            paper_gate_abs = MathMax(paper_gate_abs,0.88);
         else if(poor_candle)
   ...
   AppendPLATINCandidateEvent(now,"EVALUATED",signal.valid,signal.reason_code,signal,0.0);
   if(!signal.valid)
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
   PLATINLocalRiskPlan risk_plan;
   BuildPLATINRiskPlan(g_state,g_market,risk_plan);
   ...
   if(signal.valid && !risk_plan.allowed)
     {
      AppendPLATINCandidateEvent(now,"SIZE_BLOCK",false,risk_plan.reason_code,signal,0.0);
      AppendPLATINDecisionEvent(now,"SIZE","SKIP",risk_plan.reason_code,g_market.spread_points,signal.score,0.0,0,true,30);
      MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
      return;
     }
   ...
   if(signal.valid)
     {
      ...
      if(!exec_check.allowed)
        {
         AppendPLATINCandidateEvent(now,"PRECHECK_BLOCK",false,exec_check.reason,signal,risk_plan.lots);
         ...
         AppendPLATINDecisionEvent(now,"EXEC_PRECHECK","BLOCK",exec_check.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_check.order_check_retcode,true,30);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         return;
        }
      AppendPLATINDecisionEvent(now,"EXEC_PRECHECK","READY","PRECHECK_OK",g_market.spread_points,signal.score,risk_plan.lots,0,false);
      if(IsLocalPaperModeActive())
        {
         if(IsLocalPaperModeActive())
           {
            ...
            MbPaperOpenPosition(
               ...
               g_market.spread_points,
               now,
               ResolvePLATINPaperHoldSeconds(signal),
               signal.reason_code,
               signal.setup_type,
            ...
            MbSavePaperPosition(g_profile.symbol,g_paper_position);
            MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
            AppendPLATINCandidateEvent(now,"PAPER_OPEN",true,"PAPER_POSITION_OPENED",signal,risk_plan.lots);
            AppendPLATINDecisionEvent(now,"PAPER_OPEN","OK","PAPER_POSITION_OPENED",g_market.spread_points,signal.score,risk_plan.lots,0,false);
         }
         AppendPLATINDecisionEvent(now,"EXEC_SEND","SKIP","LIVE_SEND_DISABLED",g_market.spread_points,signal.score,risk_plan.lots,0,true,30);
        }
      else
        {
         MbMarkOrderSend(g_state);
         ...
         AppendPLATINCandidateEvent(now,(exec_result.ok ? "EXEC_SEND_OK" : "EXEC_SEND_ERROR"),exec_result.ok,exec_result.reason,signal,risk_plan.lots);
         AppendPLATINDecisionEvent(now,"EXEC_SEND",(exec_result.ok ? "OK" : "ERROR"),exec_result.reason,g_market.spread_points,signal.score,risk_plan.lots,exec_result.retcode,false);
         MbClearCandidateArbitrationSnapshot(g_profile.session_profile,g_profile.symbol);
         if(exec_result.ok)
            return;
        }
   ...
   AppendPLATINDecisionEvent(now,"SCAN",(signal.valid ? "READY" : "SKIP"),signal.reason_code,g_market.spread_points,signal.score,(signal.valid ? risk_plan.lots : 0.0),0,throttle_scan,60);
  }
